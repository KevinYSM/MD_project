...
def verify_rename(old_file_loc, new_file_loc):
        ...
        if ((re.search("PDX",old_file_loc)!=None) or (re.search("PBMC",old_file_loc)!=None)):
                old_sample_type="tumour"
        elif ((re.search("TIL",old_file_loc)!=None) or (re.search("biopsy", old_file_loc)!=None)):

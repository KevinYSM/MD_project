...
def main():
       ...
       # print(normal_fastqs[1][1])
        ...
        pairs=get_pairs(all_samples)
        create_all_samplesheet(all_samples,"/home/ubuntu/data/local/MD_project/scripts/pipelines/exome/04_samplesheets/samplesheets/disambiguate.csv")
...
def get_pairs(samples):
        ...
        for sample_1 in samples:
                if (re.search("tumis",sample_1)!=None):
                        attributes_1=get_attributes(sample_1)
                        patient_id=attributes_1["ID"]
                        pair=[]
                        
                        for sample_2 in samples:
                                if (re.search("_human_",sample_2)!=None):
                                   
                                        if (patient_id==re.match(r"\d+",sample_2).group(0)):
...
def get_fastqs(bbsplit_dir, trimmed_umis_dir):
        ...
        tumour_samples= [file for file in os.listdir(bbsplit_dir) if "tumour" in file and file.endswith(".fastq.gz") and "human" in file] 
        ...
        all_samples=tumour_samples+normal_samples_tumi_name

        
        print(len(tumour_samples),len(normal_samples_tumi_name))
...
def create_all_samplesheet(all_samples, samplesheet_name):
        finished_double=False
        normal_fastqs=get_normal_fastqs(trimmed_umis_dir)
        print(normal_fastqs)
        tumi_df=pd.read_csv(rename_table,names=["ID","lane","sample","read","batch","Raw_name","New_name","Tumi_name"])
        if (os.path.exists(samplesheet_name)):
                f=open(samplesheet_name,"w")
                f.truncate()
                f.close()
        ...
        for sample in all_samples:
                ...
                if (re.search("tumis",sample)!=None):
                        attributes=get_attributes(sample)
                        ID=attributes["ID"]
                        sex=attributes["sex"]
                        status="0"
                        lane=attributes["lane"]
                        samplesheet_sample=ID+"_normal_sample"
                        
                        #print(sample)
                        #print(re.search("_R1_",sample))
                        ...
                        if ("_R1_" in sample):
                                for f in normal_fastqs:
                                        ...
                                        if (tumi_name == sample):
                                                ...
                                                read2= re.sub(r"_R1_", "_R2_", read1)
                        ...
                        ID=re.match(r"\d+",sample).group(0)

                        sex=get_sex(ID)
                        status="1"
                        if ("_sample_2" in sample):
                                samplesheet_sample=ID+"_tumour_sample_2"
                        else:
                                samplesheet_sample=ID+"_tumour_sample"
                                
                        matching_ID=tumi_df.loc[tumi_df["ID"].str.strip()=="P"+str(ID)]
                        
                        matching_sample=matching_ID[matching_ID["sample"].str.contains("tumour")]
                        if (len(matching_sample.index)==4):
                                if (finished_double==False):
                                        matching_sample=matching_ID[matching_ID["sample"].str.contains("tumour1")]
                                        finished_double=True
                                else:
                                      matching_sample=matching_ID[matching_ID["sample"].str.contains("tumour2")]  
                        
                        if (matching_sample["lane"].tolist()[0]=="L001"):
                                lane="1"
                        else:
                                lane="2"
                        if ("_R1." in sample):
                                read1=bbsplit_dir+sample
                                read2= re.sub(r"_R1.", "_R2.", read1)
                ...
                row=str(ID+","+sex+","+status+","+ samplesheet_sample+","+lane+","+read1+","+read2)
                
                writer.writerow([row])



def create_all_samplesheet_old(all_samples):
        print(len(all_samples))
        all_csv="/home/ubuntu/data/local/MD_project/scripts/pipelines/exome/04_samplesheets/samplesheets/"+"all.csv"
        if (os.path.exists(all_csv)):
                f=open(all_csv,"w")
                f.truncate()
                f.close()

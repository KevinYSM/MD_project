...
counts <- seu.filt.cd8_t.liver@assays$RNA@counts
metadata <- seu.filt.cd8_t.liver@meta.data
# 
# counts <- seu.filt.cd8_t@assays$RNA@counts
# metadata <- seu.filt.cd8_t@meta.data

sce <- SingleCellExperiment(assays = list(counts = counts), 
                            colData = metadata)

groups <- colData(sce)[, c( "Sample.ID")]
...
cluster_counts <- t(pb)
cluster_metadata <- unique(data.frame(Sample.ID = metadata$Sample.ID, 
                               group_id = metadata$Mouse_biopsy_site_and_TIL_selection))
...
#                                    levels = c("Liver_Bulk","Liver_MART1"))
...
contrast <- c("group_id", "Liver_MART1", "Liver_Bulk")
res <- results(dds, 
               contrast = contrast,
               alpha = 0.05)
...
resASHR <- lfcShrink(dds,
                 contrast =  contrast,
                 res=res, type = "ashr")
resAPEGLM <- lfcShrink(dds, coef = "group_id_Liver_MART1_vs_Liver_Bulk", type = "apeglm")

par(mfrow=c(1,4), mar=c(4,4,2,1))
xlim <- c(1,1e5); ylim <- c(-3,5)
plotMA(resDEFAULT, xlim=xlim, ylim=ylim, main="default")
plotMA(resNorm, xlim=xlim, ylim=ylim, main="normal")
plotMA(resASHR, xlim=xlim, ylim=ylim, main="ashr")
...
#library(apeglm)
# resultsNames(dds) # 2 is the relevant contrast
...
# Plot correlation heatmap (Extract the rlog matrix from the object and compute pairwise correlation values)
library(pheatmap)
rld_mat <- assay(rld)
rld_cor <- cor(rld_mat)
...
# Heatmap
pheatmap(sig_norm, border_color = NA, scale = "row", annotation = cluster_metadata)

## Volcano plot
ggplot(as.data.frame(resDEFAULT)) +

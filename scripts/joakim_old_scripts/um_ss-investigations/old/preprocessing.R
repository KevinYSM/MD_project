...
pheatmap(table(dat.integrated.doubletfiltered.reprocecessed@meta.data$seurat_clusters,dat.integrated.doubletfiltered.reprocecessed@meta.data$active.ident))

dat.integrated.doubletfiltered.reprocecessed.bak <- dat.integrated.doubletfiltered.reprocecessed

dat.integrated.doubletfiltered.reprocecessed <- subset(dat.integrated.doubletfiltered.reprocecessed, seurat_clusters != 9)

dat.integrated.doubletfiltered.reprocecessed.annotated <- dat.integrated.doubletfiltered.reprocecessed

dat.integrated.doubletfiltered.reprocecessed.annotated <- RunPCA(
  dat.integrated.doubletfiltered.reprocecessed.annotated, verbose = T, npcs = 50)

dat.integrated.doubletfiltered.reprocecessed.annotated <- RunUMAP(
  dat.integrated.doubletfiltered.reprocecessed.annotated, reduction = "pca", dims = 1:50, 
  verbose = T)

Idents(dat.integrated.doubletfiltered.reprocecessed.annotated) <- "seurat_clusters"

dat.integrated.doubletfiltered.reprocecessed.annotated@meta.data$active.ident <- NULL

dat.integrated.doubletfiltered.reprocecessed.annotated <- RenameIdents(dat.integrated.doubletfiltered.reprocecessed.annotated, 
                               `0` = "CD4 T cell (I)", #
                               `1` = "CD8 T cell (Exhausted/dysfunctional)", #
                               `2` = "Melanocytic (MITF high)",
                               `3` = "Melanocytic (MITF, TYR high)", #
                               `4` = "CD8 T cell (GNLY+, FCGR3A+)", #
                               `5` = "NK cells", #
                               `6` = "Monocytes/macrophages/dendritic cells (ITGB2, S100A9 high)", #
                               `7` = "CD4 T cell (II)", #
                               `8` = "CD8 T cell (Exhausted/dysfunctional, cluster 8)", #
                               `10` = "CD8 T cell", #
                               `11` = "B cell", #
                               `12` = "CD8 T cell (cluster 12)", #
                               `13` = "Melanocytic (MITF, TYR, CDKN2A high)", #
                               `14` = "pDC", #
                               `15` = "Erythrocytes", #
                               `16` = "Macrophages" #
)
#`9` = "Melanocytic / T cell doublets / poor quality", #

dat.integrated.doubletfiltered.reprocecessed.annotated <- RenameIdents(dat.integrated.doubletfiltered.reprocecessed.annotated, 
                                                                       `CD8 T cell (Exhausted/dysfunctional)` = "CD8 T cell (I)", #
                                                                       `CD8 T cell (Exhausted/dysfunctional, cluster 8)` = "CD8 T cell (II)",
                                                                       `Melanocytic (MITF high)` = "Melanocytic (I)",
                                                                       `Melanocytic (MITF, TYR high)` = "Melanocytic (II)", #
                                                                       `CD8 T cell (GNLY+, FCGR3A+)` = "CD8 T cell (III)", #
                                                                       `Monocytes/macrophages/dendritic cells (ITGB2, S100A9 high)` = "Monocytes/macrophages/dendritic cells", #
                                                                       `CD8 T cell` = "CD8 T cell (IV)", #
                                                                       `CD8 T cell (cluster 12)` = "CD8 T cell (V)", #
                                                                       `Melanocytic (MITF, TYR, CDKN2A high)` = "Melanocytic (III)" #
)


DimPlot(dat.integrated.doubletfiltered.reprocecessed.annotated)
#DimPlot(dat.integrated.doubletfiltered.reprocecessed.annotated, group.by = "seurat_clusters")

outdir <- "~/proj/um_ss/Investigations/seurat/results/"
pdf(file=paste0(outdir,"dat.integrated.doubletfiltered.reprocecessed.annotated.dimplot.pdf"),width = 11,height = 10)
DimPlot(dat.integrated.doubletfiltered.reprocecessed.annotated, label = T) + theme(legend.position="none")
dev.off()

pdf(file=paste0(outdir,"dat.integrated.doubletfiltered.reprocecessed.annotated.dimplot.seurat_clusters.pdf"),width = 11,height = 10)
DimPlot(dat.integrated.doubletfiltered.reprocecessed.annotated, group.by = "seurat_clusters", label = T) + 
  theme(legend.position="none") + ggtitle(element_blank())
dev.off()

pdf(file=paste0(outdir,"dat.integrated.doubletfiltered.reprocecessed.annotated.dimplot.tcr.pdf"),width = 11,height = 10)
DimPlot(dat.integrated.doubletfiltered.reprocecessed.annotated, group.by = "cdr3", label = F) + 
  theme(legend.position="none") + ggtitle(element_blank())
dev.off()

DimPlot(dat.integrated.doubletfiltered.reprocecessed.annotated, label = F, split.by = "project.name")

DotPlot(dat.integrated.doubletfiltered.reprocecessed.annotated, features = m.to_plot, cols = c("blue", "red"), 
        dot.scale = 8,assay = "RNA") +
  RotatedAxis()

dat.integrated.doubletfiltered.reprocecessed.annotated <- AddMetaData(
  dat.integrated.doubletfiltered.reprocecessed.annotated, 
  metadata = dat.integrated.doubletfiltered.reprocecessed.annotated@active.ident, 
  col.name = "active.ident")

false_melanocytes <- (dat.integrated.doubletfiltered.reprocecessed.annotated@meta.data$project.name == "G18-023" & grepl("Melanocytic",dat.integrated.doubletfiltered.reprocecessed.annotated@meta.data$active.ident))
false_melanocytes <- setNames(false_melanocytes,rownames(dat.integrated.doubletfiltered.reprocecessed.annotated@meta.data))

dat.integrated.doubletfiltered.reprocecessed.annotated <- AddMetaData(
  dat.integrated.doubletfiltered.reprocecessed.annotated, 
  metadata = false_melanocytes, 
  col.name = "false_melanocytes")

dat.integrated.doubletfiltered.reprocecessed.annotated.bak <- dat.integrated.doubletfiltered.reprocecessed.annotated

dat.integrated.doubletfiltered.reprocecessed.annotated <- subset(dat.integrated.doubletfiltered.reprocecessed.annotated, false_melanocytes == FALSE) # accidentcally meaning the opposite
...
pdf(file=paste0(outdir,"dat.integrated.doubletfiltered.reprocecessed.annotated.dimplot.by_project.pdf"),width = 35,height = 10)
DimPlot(dat.integrated.doubletfiltered.reprocecessed.annotated, label = F, split.by = "project.name")
dev.off()

pdf(file=paste0(outdir,"dat.integrated.doubletfiltered.reprocecessed.annotated.dimplot.by_project_w_labels.pdf"),width = 33,height = 10)
DimPlot(dat.integrated.doubletfiltered.reprocecessed.annotated, label = T, 
        split.by = "project.name", label.size = 2) + 
  theme(legend.position="none")
dev.off()

#saveRDS(dat.integrated.doubletfiltered.reprocecessed.annotated,file = paste0(outdir,"dat.integrated.doubletfiltered.reprocecessed.annotated.v1.rda"))

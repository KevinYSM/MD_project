...
test_gene_wx(s,gene,pheno = "CD3+41bb+")
...
s.41bb <- lapply(snames.41bb,function(x){get_subset(x,out.41bb,pheno="CD3+41bb+")})
names(s.mart1) <- snames.mart1
names(s.41bb) <- snames.41bb
...
names(tst.wx.41bb) <- snames.41bb

sname <- "SampleID_9_11june18"
pheno <- "CD3+41bb+"

rs <- tst.wx.41bb[["SampleID_9_11june18"]]
...
write.table(sort(names(which(table(unlist(
  lapply(lapply(tst.wx.41bb,filter_fun),rownames)))>=3))),
  quote = F,row.names = F,col.names = F)


lapply(tst.wx.mart1,filter_fun)

write.table(sort(names(which(table(unlist(
  lapply(lapply(tst.wx.mart1,filter_fun),rownames)))>=2))),
  quote = F,row.names = F,col.names = F)


table(s@meta.data$takara_clonotype)[names(table(s@meta.data$takara_clonotype[s@assays$RNA@data["TRGV9",]>0]))]
table(s@meta.data$takara_clonotype[s@assays$RNA@data["TRGV9",]>0])

table(s@meta.data$takara_clonotype)[names(table(s@meta.data$takara_clonotype[s@assays$RNA@data["TRGV2",]>0]))]
table(s@meta.data$takara_clonotype[s@assays$RNA@data["TRGV2",]>0])

s@meta.data[s@assays$RNA@data["TRGV9",]>0,]

FeaturePlot(out.41bb[[1]]$y,features = c("TRDV1","TRDV2","TRGV9","TRGV10","CD8A","CD4","NCAM1"),order = T)
FeaturePlot(out.41bb[[2]]$y,features = c("TRDV1","TRDV2","TRGV9","TRGV10","CD8A","CD4","NCAM1"),order = T)
FeaturePlot(out.41bb[[3]]$y,features = c("TRDV1","TRDV2","TRGV9","TRGV10","CD8A","CD4","NCAM1"),order = T)
FeaturePlot(out.41bb[[4]]$y,features = c("TRDV1","TRDV2","TRGV9","TRGV10","CD8A","CD4","NCAM1"),order = T)

FeaturePlot(out.41bb[[4]]$y,
            features = c("TRDV1","TRDV2","TRGV9","TRGV10",
                         "CD8A","CD4","NCAM1","CD3D","MLANA",
                         "nCount_RNA"),order = T) + 
  DimPlot(out.41bb[[4]]$y,group.by = "vasu_phenotype")

FeaturePlot(subset(dat.cca,Sample.ID == "SampleID_7_11june18"),
            features = c("TRDV1","TRDV2","TRGV9","TRGV10",
                         "CD8A","CD4","NCAM1","CD3D","MLANA",
                         "nCount_RNA"),order = T) + 
DimPlot(subset(dat.cca,Sample.ID == "SampleID_9_11june18"),group.by = "vasu_phenotype")


FeaturePlot(s.41bb[["SampleID_7_11june18"]]$s,
            features = c("TRDV1","TRDV2","TRGV9","TRGV10",
                         "CD8A","CD4","NCAM1","CD3D","CD3G","MLANA",
                         "nCount_RNA"),order = T) + 
  DimPlot(s.41bb[["SampleID_7_11june18"]]$s,group.by = "vasu_phenotype")

sig_genes <- sort(names(which(table(unlist(lapply(lapply(tst.wx.41bb,filter_fun),rownames)))>=3)))[11:21]
FeaturePlot(s.41bb[["SampleID_7_11june18"]]$s,
            features = c(sig_genes,
                         "nCount_RNA"),order = T) + 
  DimPlot(s.41bb[["SampleID_7_11june18"]]$s,group.by = "vasu_phenotype")
subset(s.41bb[["SampleID_9_11june18"]]$s,cells = cell_ids2)
FeaturePlot(s.41bb[["SampleID_9_11june18"]]$s,
            features = c("TRGV10",names(which(sort(table(unlist(lapply(res.lm.41bb.filtered,rownames))),decreasing = T)>=3))[1:20],
                         "nCount_RNA"),order = T) + 
  DimPlot(subset(s.41bb[["SampleID_9_11june18"]]$s,cells = cell_ids2),group.by = "vasu_phenotype")

FeaturePlot(s.41bb[["C7-GEX"]]$s,
            features = c("LAYN","TIGIT","HAVCR2","LAG3","TNFRSF9",
                         "PDCD1","ENTPD1","CTLA4","ICOS","LAMP1",
                         "nCount_RNA"),order = T) + 
  DimPlot(s.41bb[["C7-GEX"]]$s,group.by = "vasu_phenotype")



cell_ids <- names(which(s.41bb[["C7-GEX"]]$s@assays$RNA@data["TNFRSF9",] > 0 & 
  (s.41bb[["C7-GEX"]]$s@assays$RNA@data["PDCD1",] > 0) +  
     (s.41bb[["C7-GEX"]]$s@assays$RNA@data["FASLG",] > 0) + 
     (s.41bb[["C7-GEX"]]$s@assays$RNA@data["ICOS",] > 0) + 
     (s.41bb[["C7-GEX"]]$s@assays$RNA@data["HAVCR2",] > 0) >= 2 & 
    s.41bb[["C7-GEX"]]$s$vasu_phenotype=="CD3+41bb+"))


s.41bb[["C7-GEX"]]$s@meta.data[cell_ids,]$takara_clonotype
#s.41bb[["C7-GEX"]]$s@meta.data[cell_ids,]$cdr3


cell_ids2 <- rownames(s.41bb[["SampleID_9_11june18"]]$s@meta.data)[s.41bb[["SampleID_9_11june18"]]$s@meta.data$takara_clonotype %in% s.41bb[["C7-GEX"]]$s@meta.data[cell_ids,]$takara_clonotype]

s2 <- subset(s.41bb[["SampleID_9_11june18"]]$s,cells = cell_ids2)

FeaturePlot(s2,
            features = c("LAYN","TIGIT","HAVCR2","LAG3","TNFRSF9",
                         "PDCD1","ENTPD1","CTLA4","ICOS","LAMP1",
                         "nCount_RNA","Phase"),order = T) + 
  DimPlot(s2,group.by = "vasu_phenotype")

FeaturePlot(s.41bb[["SampleID_9_11june18"]]$s,
            features = c("LAYN","TIGIT","HAVCR2","LAG3","TNFRSF9",
                         "PDCD1","ENTPD1","CTLA4","ICOS","LAMP1",
                         "nCount_RNA","G2M.Score"),order = T) + 
  DimPlot(s.41bb[["SampleID_9_11june18"]]$s,group.by = "vasu_phenotype")

FeaturePlot(out.41bb[["SampleID_9_11june18"]]$y,
            features = c("LAYN","TIGIT","HAVCR2","LAG3","TNFRSF9",
                         "PDCD1","ENTPD1","CTLA4","ICOS","LAMP1","NCAM1",
                         "CD8A","CD8B",
                         "nCount_RNA"),order = T) + 
  DimPlot(out.41bb[["SampleID_9_11june18"]]$y,group.by = "vasu_phenotype")


#CellBender
#CellTypist

FeaturePlot(subset(dat.cca,Sample.ID==snames.41bb[1]),features = c("TRGV9"),order = T) + 
  FeaturePlot(subset(dat.cca,Sample.ID==snames.41bb[2]),features = c("TRGV9"),order = T) + 
  FeaturePlot(subset(dat.cca,Sample.ID==snames.41bb[3]),features = c("TRGV9"),order = T) + 
  FeaturePlot(subset(dat.cca,Sample.ID==snames.41bb[4]),features = c("TRGV9"),order = T)
...
# Test (lm)
test_gene_lm <- function(s,genes_keep,pheno){
  res <- list()
  for (gene in genes_keep){
    res[[gene]] <- summary(lm(as.numeric(s@meta.data$vasu_phenotype == pheno) ~ 
                 log2(s@assays$RNA@data[gene,]+1) + log2(s@meta.data$nCount_RNA+1) + 
                 s@meta.data$S.Score + s@meta.data$G2M.Score + s@meta.data$percent_mito + 
                 s@meta.data$percent_ribo))
  }
  return(res)
}

res.lm.mart1 <- lapply(s.mart1,function(x){test_gene_lm(x$s,x$genes_keep,pheno="MART1")})
res.lm.41bb <- lapply(s.41bb,function(x){test_gene_lm(x$s,x$genes_keep,pheno="CD3+41bb+")})

cnames <- c(
  "(Intercept)",
  "log2(s@assays$RNA@data[gene, ] + 1)",
  "log2(s@meta.data$nCount_RNA + 1)",
  "s@meta.data$S.Score",
  "s@meta.data$G2M.Score",
  "s@meta.data$percent_mito",
  "s@meta.data$percent_ribo"
)
res.lm.41bb <- lapply(res.lm.41bb,function(x){
  do.call("rbind",lapply(x,function(y){
    tmp <- coef(y)[,"Pr(>|t|)"]
    if (! all(cnames %in% names(tmp))){
      missing_cnames <- setdiff(cnames,names(tmp))
      tmp <- c(tmp,rep(NA,length(missing_cnames)))
      names(tmp)[names(tmp)==""] <- missing_cnames
      tmp <- tmp[cnames]
    }
    return(tmp)
  }))
...
res.lm.mart1 <- lapply(res.lm.mart1,function(x){
  do.call("rbind",lapply(x,function(y){
    tmp <- coef(y)[,"Pr(>|t|)"]
    if (! all(cnames %in% names(tmp))){
      missing_cnames <- setdiff(cnames,names(tmp))
...
filter_fun_lm <- function(x){
  x[which(x[,"log2(s@assays$RNA@data[gene, ] + 1)"] < 0.05),]
}

res.lm.41bb.filtered <- lapply(res.lm.41bb,filter_fun_lm)

names(which(sort(table(unlist(lapply(res.lm.41bb.filtered,rownames))),decreasing = T)>=3))

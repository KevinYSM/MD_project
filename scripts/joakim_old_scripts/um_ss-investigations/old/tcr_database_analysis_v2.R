...
#db.matches.bak <- db.matches
#db.matches <- db.matches.bak
for (sname in names(db.matches)){
  db.matches[[sname]]$Sample.ID <- sname
  db.matches[[sname]]$UM.ID <- unique(dat.cca@meta.data$UM.ID[dat.cca@meta.data$Sample.ID==sname])
  db.matches[[sname]]$project.name <- unique(dat.cca@meta.data$project.name[dat.cca@meta.data$Sample.ID==sname])
}
...
unique(db.matches$Species)

tmp <- db.matches[,c("Antigen","UM.ID","project.name","cell")]
tmp$Antigen[tmp$Antigen %in% c("MelanA/MART1","MLANA","Melan-A/MART-1")] <- "MART1"
dim(tmp)
dim(unique(tmp))
tmp <- unique(tmp)
...
annot_row <- data.frame(Antigen=db.matches$Antigen,origin=db.matches$Antigen_species_or_pathology)
annot_row$Antigen[annot_row$Antigen %in% c("MelanA/MART1","MLANA","Melan-A/MART-1")] <- "MART1"
annot_row <- unique(annot_row)
annot_row <- annot_row[!is.na(annot_row$Antigen),]
table(annot_row$Antigen,annot_row$origin)
annot_row[annot_row$Antigen %in% names(which(table(annot_row$Antigen)>1)),]
annot_row$origin[annot_row$origin=="Cytomegalovirus (CMV)"] <- "CMV"
annot_row$origin[annot_row$origin=="InfluenzaA"] <- "Influenza"
annot_row$origin[annot_row$origin=="Melanoma"] <- "HomoSapiens"
annot_row <- unique(annot_row)
rownames(annot_row) <- annot_row$Antigen
annot_row$Antigen <- NULL
unique(annot_row$origin)

annot_row

library(pheatmap)
pheatmap(m,border_color = NA,na_col = "white",colorRampPalette(c("white", "red"))(100),
         annotation_row = annot_row,display_numbers = m,treeheight_row = 0,treeheight_col = 100,
         cluster_cols = F)

m.s <- m[rownames(m) %in% rownames(annot_row)[annot_row$origin %in% c("Melanoma","HomoSapiens","Human")],]
pheatmap(m.s,border_color = NA,na_col = "white",colorRampPalette(c("white", "red"))(100),
         annotation_row = annot_row,display_numbers = m.s)

m.s <- m[rownames(m) %in% rownames(annot_row)[annot_row$origin %in% 
                                                c("Celiac disease","TriticumAestivum","Diabetes Type 1","SARS-CoV-2")],]
pheatmap(m.s,border_color = NA,na_col = "white",colorRampPalette(c("white", "red"))(100),
         annotation_row = annot_row,display_numbers = m.s)
...
df.merge.candidates.db_match[["1_TCACAAGAGAATGTTG_1"]]
dat.cca@meta.data["1_TCACAAGAGAATGTTG_1",c("cdr3","v_gene","d_gene","j_gene","c_gene")]

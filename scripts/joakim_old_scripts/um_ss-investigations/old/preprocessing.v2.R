...
# Reintegrate after further filtering (RPCA instead) ---------------------------
...
anchors <- FindIntegrationAnchors(object.list = dat.split,
                                  normalization.method = "SCT",
                                  anchor.features = features, verbose = T, 
                                  dims = 1:50, n.trees = 100, reduction = "rpca")
...
saveRDS(dat.split,file="/data/proj/um_ss/Investigations/seurat/results/dat.integrated.doubletfiltered.reprocecessed.filtered.reprocessed.rpca.rda")

# RPCA with higher k.anchor (20) (https://satijalab.org/seurat/articles/integration_rpca.html) ------
...
saveRDS(dat.split,file="/data/proj/um_ss/Investigations/seurat/results/dat.integrated.doubletfiltered.reprocecessed.filtered.reprocessed.rpca.k.anchor_20.rda")

# RPCA with higher k.anchor (10) (https://satijalab.org/seurat/articles/integration_rpca.html) ------
...
saveRDS(dat.split,file="/data/proj/um_ss/Investigations/seurat/results/dat.integrated.doubletfiltered.reprocecessed.filtered.reprocessed.rpca.k.anchor_10.rda")
...
dat.rpca <- readRDS("~/proj/um_ss/Investigations/seurat/results/dat.integrated.doubletfiltered.reprocecessed.filtered.reprocessed.rpca.rda")
dat.rpca.k.anchor_20 <- readRDS("~/proj/um_ss/Investigations/seurat/results/dat.integrated.doubletfiltered.reprocecessed.filtered.reprocessed.rpca.k.anchor_20.rda")
dat.rpca.k.anchor_10 <- readRDS("~/proj/um_ss/Investigations/seurat/results/dat.integrated.doubletfiltered.reprocecessed.filtered.reprocessed.rpca.k.anchor_10.rda")

markers_plot <- readRDS(paste0(outdir,"markers_plot.rda"))

#dat.rpca.candidate_labels
dat.rpca <- RenameIdents(
  dat.rpca,
  `0` = "Melanocytic (I)",
  `1` = "CD8 T cells (II)",
  `2` = "CD4 T cells (I)",
  ...
  `17` = "NK cells (II)",
  `18` = "CD8 T cells (X)",
  `19` = "Gamma-delta T cells (TRDV2, TRGV9)",
  `20` = "CD8 T cells (II)",
  `21` = "CD8 T cells (VIII)",
  `22` = "Melanocytic (VI)",
  `23` = "CD8 T cells (VII)",
...
#dat.rpca.k.anchor_20.candidate_labels
dat.rpca.k.anchor_20 <- RenameIdents(
  dat.rpca.k.anchor_20,
  `0` = "Melanocytic (I)",
  `1` = "CD8 T cells (IV)",
  `2` = "CD4 T cells (IV)",
  `3` = "CD4 T cells (II)",
  ...
  `23` = "Melanocytic (VI)",
  `24` = "CD8 T cells (VII)",
  `25` = "Gamma-delta T cells (TRDV2, TRGV9)",
  `26` = "Melanocytic (V)",
  `27` = "CD4 T cells (III)",
  `28` = "CD16 Monocytes",
  `29` = "Melanocytic (IV)",
...
#dat.cca.candidate_labels
dat.cca <- RenameIdents(
  dat.cca,
  `0` = "Melanocytic (I)",
  `1` = "CD4 T cells (I)",
  `2` = "CD8 T cells (I)",
...
levels(x = dat.cca) <- sort(unique(as.character(dat.cca@active.ident)))
levels(x = dat.rpca) <- sort(unique(as.character(dat.rpca@active.ident)))
levels(x = dat.rpca.k.anchor_20) <- sort(unique(as.character(dat.rpca.k.anchor_20@active.ident)))
#levels(x = dat.rpca.k.anchor_10) <- sort(unique(as.character(dat.rpca.k.anchor_10@active.ident)))

dat.cca <- AddMetaData(dat.cca, metadata = dat.cca@active.ident,
                        col.name = "active.ident.updated_3")

dat.rpca <- AddMetaData(dat.rpca, metadata = dat.rpca@active.ident,
                                    col.name = "active.ident.updated_3")

dat.rpca.k.anchor_20 <- AddMetaData(dat.rpca.k.anchor_20, metadata = dat.rpca.k.anchor_20@active.ident,
                                    col.name = "active.ident.updated_3")

plot_clustering <- function(dat, nm, outdir, group_by){
  
  #DimPlot
  pdf(file = paste0(outdir,"dat.",nm,".pdf"),width = 15,height = 10)
  print(DimPlot(dat, label = T, reduction = "tsne", group.by = group_by))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".project.pdf"),width = 35,height = 10)
  print(DimPlot(dat, label = T, reduction = "tsne", split.by = "project.name",
          group.by = group_by))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".clusters.pdf"),width = 15,height = 10)
  print(DimPlot(dat, label = T, reduction = "tsne"))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".clusters.project.pdf"),width = 35,height = 10)
  print(DimPlot(dat, label = T, reduction = "tsne", split.by = "project.name"))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".project.sex.pdf"),width = 35,height = 10)
  print(DimPlot(dat, label = T, reduction = "tsne", split.by = "project.name", group.by = "Sex"))
  dev.off()
  
  #DotPlot
  pdf(file = paste0(outdir,"dat.",nm,".dotplot.seurat_clusters.G18-023.pdf"),width = 33,height = 12)
  print(DotPlot(subset(dat, project.name == "G18-023"), 
          features = markers_plot,
          cols = c("blue", "red"), dot.scale = 8, assay = "RNA", group.by = "seurat_clusters") +
    RotatedAxis())
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".dotplot.seurat_clusters.G18-049.pdf"),width = 33,height = 12)
  print(DotPlot(subset(dat, project.name == "G18-049"), 
          features = markers_plot,
          cols = c("blue", "red"), dot.scale = 8, assay = "RNA", group.by = "seurat_clusters") +
    RotatedAxis())
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".dotplot.seurat_clusters.GWA-JN-388.pdf"),width = 33,height = 12)
  print(DotPlot(subset(dat, project.name == "GWA-JN-388"), 
          features = markers_plot,
          cols = c("blue", "red"), dot.scale = 8, assay = "RNA", group.by = "seurat_clusters") +
    RotatedAxis())
  dev.off()
  ...
  # Pheatmap
  #stopifnot(identical(as.numeric(dat@active.ident),as.numeric(dat@meta.data$seurat_clusters)))
  pdf(file = paste0(outdir,"dat.",nm,".pheatmap.pdf"),width = 10,height = 8)
  x <- table(dat@meta.data$seurat_clusters,dat@meta.data[[group_by]])
  x[x==0] <- NA
  dat.candidate_labels <- setNames(rownames(x),colnames(x)[apply(x,1,function(y){which.max(y)})])
  annot <- data.frame(names(dat.candidate_labels),
                      row.names=as.character(dat.candidate_labels))
  print(pheatmap(x,border_color = NA,cluster_cols = F,cluster_rows = F,na_col = "#FFFFFF",
           colorRampPalette(c("white", "#DC143C"))(100),
  ...
  meta <- dat@meta.data
  meta[["active.ident.updated_X"]] <- meta[[group_by]]
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_nFeatures.G18-023.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "G18-023",],
         aes(x=active.ident.updated_X,y=nFeature_RNA)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_nFeatures.G18-049.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "G18-049",],
         aes(x=active.ident.updated_X,y=nFeature_RNA)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_nFeatures.GWA-JN-388.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "GWA-JN-388",],
         aes(x=active.ident.updated_X,y=nFeature_RNA)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_mito.G18-023.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "G18-023",],
         aes(x=active.ident.updated_X,y=percent_mito)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_mito.G18-049.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "G18-049",],
         aes(x=active.ident.updated_X,y=percent_mito)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_mito.GWA-JN-388.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "GWA-JN-388",],
         aes(x=active.ident.updated_X,y=percent_mito)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  ...
  # Seurat clusters
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_nFeatures.seurat_clusters.G18-023.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "G18-023",],
         aes(x=seurat_clusters,y=nFeature_RNA)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_nFeatures.seurat_clusters.G18-049.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "G18-049",],
         aes(x=seurat_clusters,y=nFeature_RNA)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_nFeatures.seurat_clusters.GWA-JN-388.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "GWA-JN-388",],
         aes(x=seurat_clusters,y=nFeature_RNA)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_mito.seurat_clusters.G18-023.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "G18-023",],
         aes(x=seurat_clusters,y=percent_mito)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_mito.seurat_clusters.G18-049.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "G18-049",],
         aes(x=seurat_clusters,y=percent_mito)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  pdf(file = paste0(outdir,"dat.",nm,".filtered.t_cell_mito.seurat_clusters.GWA-JN-388.pdf"),width = 10,height = 7)
  print(ggplot(meta[meta$project.name == "GWA-JN-388",],
         aes(x=seurat_clusters,y=percent_mito)) + geom_violin(draw_quantiles = 0.5) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
  dev.off()
  
  return(dat.candidate_labels)
}

get_candidate_labels <- function(dat,group_by){
  x <- table(dat@meta.data$seurat_clusters,dat@meta.data[[group_by]])
  x[x==0] <- NA
  dat.candidate_labels <- setNames(rownames(x),colnames(x)[apply(x,1,function(y){which.max(y)})])
  return(dat.candidate_labels)
...
dat.rpca.k.anchor_20.resolution_1 <- FindClusters(
  dat.rpca.k.anchor_20, resolution = 1.0, verbose = T)

dat.rpca.k.anchor_20.resolution_1.candidate_labels <- plot_clustering(dat = dat.rpca.k.anchor_20.resolution_1, 
                nm = "rpca.k.anchor_20.resolution_1", 
                outdir = outdir, 
                group_by = "active.ident.updated_3")

dat.rpca.k.anchor_20.resolution_09 <- FindClusters(
  dat.rpca.k.anchor_20, resolution = 0.9, verbose = T)

plot_clustering(dat = dat.rpca.k.anchor_20.resolution_09,
                nm = "rpca.k.anchor_20.resolution_09", 
                outdir = outdir, 
                group_by = "active.ident.updated_3")

dat.rpca.k.anchor_20.resolution_095 <- FindClusters(
  dat.rpca.k.anchor_20, resolution = 0.95, verbose = T)

plot_clustering(dat = dat.rpca.k.anchor_20.resolution_095,
                nm = "rpca.k.anchor_20.resolution_095", 
                outdir = outdir, 
                group_by = "active.ident.updated_3")

# plot_clustering(dat = dat.rpca,
#                 nm = "rpca", 
#                 outdir = outdir, 
#                 group_by = "active.ident.updated_2")

dat.cca.candidate_labels <- plot_clustering(dat = dat.cca,
                nm = "cca", 
                outdir = outdir, 
                group_by = "active.ident.updated_3")

dat.cca.resolution_1 <- FindClusters(
  dat.cca, resolution = 1.0, verbose = T)

plot_clustering(dat = dat.cca.resolution_1,
                nm = "cca.resolution_1", 
                outdir = outdir, 
                group_by = "active.ident.updated_3")


plot_clustering(dat = dat.rpca.k.anchor_10,
                nm = "rpca.k.anchor_10", 
                outdir = outdir, 
                group_by = "active.ident.updated_2")

dat.rpca.k.anchor_10.resolution_1 <- FindClusters(
  dat.rpca.k.anchor_10, resolution = 1.0, verbose = T)

dat.rpca.k.anchor_10.resolution_1.candidate_labels <- get_candidate_labels(
  dat = dat.rpca.k.anchor_10.resolution_1,
  group_by = "active.ident.updated_2")


dat.rpca.k.anchor_10.resolution_1.candidate_labels <- plot_clustering(dat = dat.rpca.k.anchor_10.resolution_1,
                nm = "rpca.k.anchor_10.resolution_1", 
                outdir = outdir, 
                group_by = "active.ident.updated_2")

dat.rpca.k.anchor_10.resolution_11 <- FindClusters(
  dat.rpca.k.anchor_10, resolution = 1.1, verbose = T)

plot_clustering(dat = dat.rpca.k.anchor_10.resolution_11,
                nm = "rpca.k.anchor_10.resolution_11", 
                outdir = outdir, 
                group_by = "active.ident.updated_2")

dat.rpca.k.anchor_10.resolution_11.algorithm_3 <- FindClusters(
  dat.rpca.k.anchor_10, resolution = 1.1, algorithm = 3, verbose = T)

plot_clustering(dat = dat.rpca.k.anchor_10.resolution_11.algorithm_3,
                nm = "rpca.k.anchor_10.resolution_11.algorithm_3", 
                outdir = outdir, 
                group_by = "active.ident.updated_2")

dat.rpca.k.anchor_10.resolution_15.algorithm_3 <- FindClusters(
  dat.rpca.k.anchor_10, resolution = 1.5, algorithm = 3, verbose = T)

plot_clustering(dat = dat.rpca.k.anchor_10.resolution_15.algorithm_3,
                nm = "rpca.k.anchor_10.resolution_15.algorithm_3", 
                outdir = outdir, 
                group_by = "active.ident.updated_2")


dat.rpca.k.anchor_10.resolution_1.candidate_labels

dat.rpca.k.anchor_10.resolution_1 <- RenameIdents(
  dat.rpca.k.anchor_10.resolution_1,
  `0` = "CD8 T cells (I)",
  `1` = "CD4 T cells (I)",
  `2` = "Melanocytic (I)",
  `3` = "CD8 T cells (II)",
  ...
  `26` = "Melanocytic (V)",
  `27` = "CD8 T cells (XIII)",
  `28` = "Gamma-delta T cells (mixed)",
  `29` = "Gamma-delta T cells (TRDV2, TRGV9)",
  `30` = "CD14 Monocytes / DCs (II)",
  `31` = "CD8 T cells (XIV)",
  `32` = "Melanocytic (VI)",
  `33` = "CD16 Monocytes",
  `34` = "Melanocytic (VII)",
  ...
  `43` = "NK cells (III)",
  `44` = "CD8 T cells (XVII)",
  `45` = "CD8 T cells (XVIII)",
  `46` = "CD8 T cells (XIX)"
)

levels(x = dat.rpca.k.anchor_10.resolution_1) <- sort(unique(as.character(dat.rpca.k.anchor_10.resolution_1@active.ident)))

dat.rpca.k.anchor_10.resolution_1 <- AddMetaData(dat.rpca.k.anchor_10.resolution_1,
                                                 metadata = dat.rpca.k.anchor_10.resolution_1@active.ident,
                                                 col.name = "active.ident.updated_3")

plot_clustering(dat = dat.rpca.k.anchor_10.resolution_1,
                nm = "dat.rpca.k.anchor_10.resolution_1.relabeled", 
                outdir = outdir, 
                group_by = "active.ident.updated_3")
...
pdf(file = paste0(outdir,"dat.",nm,".dotplot.ident.G18-023.pdf"),width = 33,height = 12)
print(DotPlot(subset(dat, project.name == "G18-023"), 
              features = markers_plot,
              cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
        RotatedAxis())
...
pdf(file = paste0(outdir,"dat.",nm,".dotplot.ident.G18-049.pdf"),width = 33,height = 12)
print(DotPlot(subset(dat, project.name == "G18-049"), 
              features = markers_plot,
              cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
        RotatedAxis())
...
pdf(file = paste0(outdir,"dat.",nm,".dotplot.ident.GWA-JN-388.pdf"),width = 33,height = 12)
print(DotPlot(subset(dat, project.name == "GWA-JN-388"), 
              features = markers_plot,
              cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
        RotatedAxis())
...
# Further filtering of misclassified cells -------------------------------------
reintegrate <- function(dat.cca.tmp,sample.tree=matrix(c(-3, 1, -2, -1),ncol=2),
                        vars.to.regress=c("S.Score", "G2M.Score"),split.by = "project.name"){
  dat.cca.tmp.sct.split <- GetAssayData(dat.cca.tmp, slot = "counts", assay = "RNA")
  dat.cca.tmp.sct.split <- CreateSeuratObject(dat.cca.tmp.sct.split, 
                                              meta.data = dat.cca.tmp@meta.data)
  dat.cca.tmp.sct.split <- SplitObject(dat.cca.tmp.sct.split, split.by = split.by)
  
  dat.cca.tmp.sct.split <- lapply(dat.cca.tmp.sct.split,
                                  function(x){
                                    SCTransform(x, vst.flavor = "v2", verbose = T,
  ...
  features <- SelectIntegrationFeatures(object.list = dat.cca.tmp.sct.split, nfeatures = 3000)
  
  dat.cca.tmp.sct.split <- PrepSCTIntegration(
    object.list = dat.cca.tmp.sct.split, 
    anchor.features = features, verbose = T)
  
  anchors <- FindIntegrationAnchors(object.list = dat.cca.tmp.sct.split,
                                    normalization.method = "SCT",
                                    anchor.features = features, verbose = T, dims = 1:50, n.trees = 100)
  dat.cca.tmp.sct.split <- IntegrateData(
    anchorset = anchors, normalization.method = "SCT",verbose = T, dims = 1:50, 
    sample.tree = sample.tree)
  dat.cca.tmp.sct.split <- RunPCA(
    dat.cca.tmp.sct.split, verbose = T,npcs = 50)
  dat.cca.tmp.sct.split <- RunUMAP(
    dat.cca.tmp.sct.split, reduction = "pca", dims = 1:50, 
    verbose = T)
  dat.cca.tmp.sct.split <- RunTSNE(
    dat.cca.tmp.sct.split,dims = 1:50, verbose = T)
  dat.cca.tmp.sct.split <- FindNeighbors(
    dat.cca.tmp.sct.split, reduction = "pca", dims = 1:50, n.trees = 100,
    verbose = T)
  dat.cca.tmp.sct.split <- FindClusters(
    dat.cca.tmp.sct.split, resolution = 1, verbose = T)
  
  return(dat.cca.tmp.sct.split)
}

make_sample_tree <- function(integration_order){
  sample.tree <- rbind(-integration_order[1:2],
                       cbind(rep(1:(length(integration_order)-2)),-integration_order[3:length(integration_order)])
  )
  return(sample.tree)
}
...
# dat.cca is selected
nm <- "cca"
pdf(file = paste0(outdir,"dat.",nm,".clusters.pdf"),width = 12,height = 10)
DimPlot(dat.cca, label = T, reduction = "tsne", group.by = "seurat_clusters")
dev.off()
...
nm <- "cca"
pdf(file = paste0(outdir,"dat.",nm,".idents.pdf"),width = 15,height = 10)
DimPlot(dat.cca, label = T, reduction = "tsne")
dev.off()


pdf(file = paste0(outdir,"dat.",nm,".idents.project.pdf"),width = 33,height = 10)
DimPlot(dat.cca, label = T, reduction = "tsne",split.by = "project.name")
dev.off()

dat.cca.gwa <- subset(dat.cca, project.name == "GWA-JN-388")
dat.cca.blood <- subset(dat.cca, project.name == "G18-049")
dat.cca.tils <- subset(dat.cca, project.name == "G18-023")

pdf(file = paste0(outdir,"dat.","cca",".dotplot.ident.relabled.GWA-JN-388.pdf"),width = 12,height = 8)
DotPlot(dat.cca.gwa, 
        features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
                     "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
                     "TSPAN13","IL3RA","CLEC4C"),
        cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
  RotatedAxis()
dev.off()

pdf(file = paste0(outdir,"dat.","cca",".dotplot.ident.relabled.G18-049.pdf"),width = 12,height = 8)
DotPlot(dat.cca.blood, 
        features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
                     "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
                     "TSPAN13","IL3RA","CLEC4C"),
        cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
...
pdf(file = paste0(outdir,"dat.","cca",".dotplot.ident.relabled.G18-023.pdf"),width = 12,height = 8)
DotPlot(dat.cca.tils, 
        features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
                     "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
                     "TSPAN13","IL3RA","CLEC4C"),
        cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
...
# dat.rpca.k.anchor_10.resolution_1.gwa <- subset(dat.rpca.k.anchor_10.resolution_1, project.name == "GWA-JN-388")
# dat.rpca.k.anchor_10.resolution_1.blood <- subset(dat.rpca.k.anchor_10.resolution_1, project.name == "G18-049")
# dat.rpca.k.anchor_10.resolution_1.tils <- subset(dat.rpca.k.anchor_10.resolution_1, project.name == "G18-023")
# 
# pdf(file = paste0(outdir,"dat.rpca.k.anchor_10.resolution_1",".dotplot.ident.relabled.GWA-JN-388.pdf"),width = 12,height = 8)
# DotPlot(dat.rpca.k.anchor_10.resolution_1.gwa,
#         features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
#                      "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
#                      "TSPAN13","IL3RA","CLEC4C"),
#         cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
...
# dev.off()
# 
# pdf(file = paste0(outdir,"dat.rpca.k.anchor_10.resolution_1",".dotplot.ident.relabled.G18-049.pdf"),width = 12,height = 8)
# DotPlot(dat.rpca.k.anchor_10.resolution_1.blood,
#         features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
#                      "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
#                      "TSPAN13","IL3RA","CLEC4C"),
#         cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
...
# dev.off()
# 
# pdf(file = paste0(outdir,"dat.rpca.k.anchor_10.resolution_1",".dotplot.ident.relabled.G18-023.pdf"),width = 12,height = 8)
# DotPlot(dat.rpca.k.anchor_10.resolution_1.tils,
#         features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
#                      "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
#                      "TSPAN13","IL3RA","CLEC4C"),
#         cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
...
t_cell_melanocyte_mixed <- c("CD4 T cells (VI)",
                             "CD8 T cells (IV)",
                             "CD8 T cells (XII)")

# Suboptimal distinction between CD4 and CD8 T cells in all three datasets

DefaultAssay(dat.cca.gwa) <- "RNA"

FeatureScatter(dat.cca.gwa,feature1 = "CD3D",feature2 = "MLANA",slot = "counts")

gene_ban <- "MLANA"
table(dat.cca.gwa@assays$RNA@counts["CD3D",] > 0, dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0)
table(dat.cca.gwa@assays$RNA@counts["CD3G",] > 0, dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0)
table(dat.cca.gwa@assays$RNA@counts["CD8A",] > 0, dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0)
table(dat.cca.gwa@assays$RNA@counts["CD8B",] > 0, dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0)
table(dat.cca.gwa@assays$RNA@counts["CD4",] > 0, dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0)
table(dat.cca.gwa@assays$RNA@counts["FCGR3A",] > 0, dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0)
table(dat.cca.gwa@assays$RNA@counts["MS4A1",] > 0, dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0)

idx <- (dat.cca.gwa@assays$RNA@counts["CD3D",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) | 
  (dat.cca.gwa@assays$RNA@counts["CD3G",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) |
  (dat.cca.gwa@assays$RNA@counts["CD8A",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) |
  (dat.cca.gwa@assays$RNA@counts["CD8B",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) |
  (dat.cca.gwa@assays$RNA@counts["CD4",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) | 
  (dat.cca.gwa@assays$RNA@counts["FCGR3A",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) | 
  (dat.cca.gwa@assays$RNA@counts["MS4A1",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0)
sum(idx)
...
idx <- idx | (dat.cca.gwa@assays$RNA@counts["CD3D",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) | 
  (dat.cca.gwa@assays$RNA@counts["CD3G",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) |
  (dat.cca.gwa@assays$RNA@counts["CD8A",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) |
  (dat.cca.gwa@assays$RNA@counts["CD8B",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) |
  (dat.cca.gwa@assays$RNA@counts["CD4",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) | 
...
idx <- idx | (dat.cca.gwa@assays$RNA@counts["CD3D",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) | 
  ...
  (dat.cca.gwa@assays$RNA@counts["CD8B",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0) |
  (dat.cca.gwa@assays$RNA@counts["MS4A1",] > 0 & dat.cca.gwa@assays$RNA@counts[gene_ban,] > 0)
sum(idx) # Note that CD4 and MS4A1 is excluded here, since MITF and CD4 and/or MS4A1 are expressed in monocytes

# gene_ban <- "MITF"
# table(dat.cca.blood@assays$RNA@counts["CD3D",] > 0, dat.cca.blood@assays$RNA@counts[gene_ban,] > 0)
# table(dat.cca.blood@assays$RNA@counts["CD3G",] > 0, dat.cca.blood@assays$RNA@counts[gene_ban,] > 0)
# table(dat.cca.blood@assays$RNA@counts["CD8A",] > 0, dat.cca.blood@assays$RNA@counts[gene_ban,] > 0)
# table(dat.cca.blood@assays$RNA@counts["CD8B",] > 0, dat.cca.blood@assays$RNA@counts[gene_ban,] > 0)
# table(dat.cca.blood@assays$RNA@counts["CD4",] > 0, dat.cca.blood@assays$RNA@counts[gene_ban,] > 0)
# 
# gene_ban <- "PMEL"
# thresh <- 1 # Shows that any expression here is just one read and probably not read
# table(dat.cca.tils@assays$RNA@counts["CD3D",] > 0, dat.cca.tils@assays$RNA@counts[gene_ban,] > thresh)
# table(dat.cca.tils@assays$RNA@counts["CD3G",] > 0, dat.cca.tils@assays$RNA@counts[gene_ban,] > thresh)
# table(dat.cca.tils@assays$RNA@counts["CD8A",] > 0, dat.cca.tils@assays$RNA@counts[gene_ban,] > thresh)
# table(dat.cca.tils@assays$RNA@counts["CD8B",] > 0, dat.cca.tils@assays$RNA@counts[gene_ban,] > thresh)
# table(dat.cca.tils@assays$RNA@counts["CD4",] > 0, dat.cca.tils@assays$RNA@counts[gene_ban,] > thresh)

cells_ban <- rownames(dat.cca.gwa@meta.data)[idx]

dat.cca <- subset(dat.cca, cells = cells_ban, invert = TRUE)
dat.cca.gwa <- subset(dat.cca, project.name == "GWA-JN-388")
...
dat.cca <- AddMetaData(dat.cca,
                       metadata = dat.cca@active.ident,
                       col.name = "active.ident.updated_4")

dat.cca.tmp <- subset(dat.cca, active.ident.updated_4 == "CD4 T cells (V)")

pdf(file = paste0(outdir,"dat.",nm,".idents.CD4_V.pdf"),width = 15,height = 10)
DimPlot(dat.cca, label = T, reduction = "tsne",
        cells.highlight = rownames(dat.cca@meta.data)[dat.cca@meta.data$active.ident.updated_4 == "CD4 T cells (V)"])
dev.off()

pdf(file = paste0(outdir,"dat.",nm,".idents.CD4_V.project.pdf"),width = 33,height = 10)
DimPlot(dat.cca, label = T, reduction = "tsne",
        cells.highlight = rownames(dat.cca@meta.data)[dat.cca@meta.data$active.ident.updated_4 == "CD4 T cells (V)"],
        split.by = "project.name")
dev.off()

pdf(file = paste0(outdir,"dat.",nm,".idents.CD4_V.project.2.pdf"),width = 12,height = 10)
DimPlot(dat.cca.tmp, label = T, reduction = "tsne",group.by = "project.name")
dev.off()

dat.cca.tmp <- FindNeighbors(dat.cca.tmp, reduction = "pca", dims = 1:50)
dat.cca.tmp <- FindClusters(dat.cca.tmp, resolution = 0.2,verbose = T)
DimPlot(dat.cca.tmp, label = T, reduction = "tsne",group.by = "seurat_clusters")
DimPlot(dat.cca.tmp, label = T, reduction = "tsne",group.by = "project.name")


dat.cca.tmp.gwa <- subset(dat.cca.tmp, project.name == "GWA-JN-388")
dat.cca.tmp.blood <- subset(dat.cca.tmp, project.name == "G18-049")
dat.cca.tmp.tils <- subset(dat.cca.tmp, project.name == "G18-023")

dat.cca.tmp.gwa <- RunPCA(dat.cca.tmp.gwa, npcs = 50)
dat.cca.tmp.gwa <- RunTSNE(dat.cca.tmp.gwa, dims = 1:50)
dat.cca.tmp.gwa <- FindNeighbors(dat.cca.tmp.gwa, reduction = "pca", dims = 1:50)
dat.cca.tmp.gwa <- FindClusters(dat.cca.tmp.gwa, resolution = 1,verbose = T)
DimPlot(dat.cca.tmp.gwa, label = T, reduction = "tsne",group.by = "seurat_clusters")
DimPlot(dat.cca.tmp.gwa, label = T, reduction = "tsne",group.by = "project.name")

dat.cca.tmp.blood <- RunPCA(dat.cca.tmp.blood, npcs = 50)
dat.cca.tmp.blood <- RunTSNE(dat.cca.tmp.blood, dims = 1:50)
dat.cca.tmp.blood <- FindNeighbors(dat.cca.tmp.blood, reduction = "pca", dims = 1:50)
dat.cca.tmp.blood <- FindClusters(dat.cca.tmp.blood, resolution = 1,verbose = T)
DimPlot(dat.cca.tmp.blood, label = T, reduction = "tsne",group.by = "seurat_clusters")
DimPlot(dat.cca.tmp.blood, label = T, reduction = "tsne",group.by = "project.name")

dat.cca.tmp.tils <- RunPCA(dat.cca.tmp.tils, npcs = 50)
dat.cca.tmp.tils <- RunTSNE(dat.cca.tmp.tils, dims = 1:50)
dat.cca.tmp.tils <- FindNeighbors(dat.cca.tmp.tils, reduction = "pca", dims = 1:50)
dat.cca.tmp.tils <- FindClusters(dat.cca.tmp.tils, resolution = 1,verbose = T)
DimPlot(dat.cca.tmp.tils, label = T, reduction = "tsne",group.by = "seurat_clusters")
DimPlot(dat.cca.tmp.tils, label = T, reduction = "tsne",group.by = "project.name")


DotPlot(dat.cca.tmp.gwa, 
        features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
                     "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
                     "TSPAN13","IL3RA","CLEC4C"),
        cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
...
DotPlot(subset(dat.cca.tmp, project.name == "GWA-JN-388"), 
        features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
                     "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
                     "TSPAN13","IL3RA","CLEC4C"),
        cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
...
DotPlot(subset(dat.cca.tmp, project.name == "G18-049"), 
        features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
                     "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
                     "TSPAN13","IL3RA","CLEC4C"),
        cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
...
DotPlot(subset(dat.cca.tmp, project.name == "G18-023"), 
        features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
                     "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
                     "TSPAN13","IL3RA","CLEC4C"),
        cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
...
idx_reclassify_to_cd8 <- dat.cca.tmp@assays$RNA@counts["CD8A",] > dat.cca.tmp@assays$RNA@counts["CD4",] | 
  dat.cca.tmp@assays$RNA@counts["CD8B",] > dat.cca.tmp@assays$RNA@counts["CD4",]
table(idx_reclassify_to_cd8)

table(dat.cca.tmp@assays$RNA@counts["CD4",idx_reclassify_to_cd8] > 0)
max(dat.cca.tmp@assays$RNA@counts["CD4",idx_reclassify_to_cd8])
min(dat.cca.tmp@assays$RNA@counts["CD4",idx_reclassify_to_cd8])

cells_reclassify <- colnames(dat.cca.tmp@assays$RNA@counts[,idx_reclassify_to_cd8])

RenameIdentsCells <- function(dat,cells_reclassify,new.label){
  active.ident <- as.character(Idents(dat))
  names(active.ident) <- rownames(dat@meta.data)
  active.ident[cells_reclassify] <- new.label
  dat <- AddMetaData(dat,metadata = active.ident, col.name = "active.ident.updated")
  Idents(dat) <- dat$active.ident.updated
  return(dat)
}

dat.cca.tmp <- RenameIdentsCells(dat.cca.tmp, cells_reclassify, "CD8 T cells (unknown)")

pheatmap(as.matrix(dat.cca.tmp.tils@assays$RNA@counts[c("CD3D","CD4","CD8A","CD8B"),
          dat.cca.tmp.tils@active.ident=="CD8 T cells (unknown)"]),show_colnames = F)

pheatmap(as.matrix(dat.cca.tmp.blood@assays$RNA@counts[c("CD3D","CD4","CD8A","CD8B"),
          dat.cca.tmp.blood@active.ident=="CD8 T cells (X)"]),show_colnames = F)


dat.cca.tmp.tils <- subset(dat.cca.tmp, project.name == "G18-023")
idx_reclassify_to_cd8 <- dat.cca.tmp.tils@assays$RNA@counts["CD8A",] > dat.cca.tmp.tils@assays$RNA@counts["CD4",] | 
  dat.cca.tmp.tils@assays$RNA@counts["CD8B",] > dat.cca.tmp.tils@assays$RNA@counts["CD4",]
table(idx_reclassify_to_cd8,dat.cca.tmp.tils@active.ident)


dat.cca.tmp.blood <- subset(dat.cca.tmp, project.name == "G18-049")
idx_reclassify_to_cd8 <- dat.cca.tmp.blood@assays$RNA@counts["CD8A",] > dat.cca.tmp.blood@assays$RNA@counts["CD4",] | 
  dat.cca.tmp.blood@assays$RNA@counts["CD8B",] > dat.cca.tmp.blood@assays$RNA@counts["CD4",]
table(idx_reclassify_to_cd8)
table(idx_reclassify_to_cd8,dat.cca.tmp.blood@active.ident)
...
idx_reclassify_to_cd8 <- (dat.cca.tmp@assays$RNA@counts["CD8A",] > dat.cca.tmp@assays$RNA@counts["CD4",] | 
  dat.cca.tmp@assays$RNA@counts["CD8B",] > dat.cca.tmp@assays$RNA@counts["CD4",]) & grepl("CD4 T cells",dat.cca.tmp@active.ident)
table(idx_reclassify_to_cd8[grepl("CD4 T cells",dat.cca.tmp@active.ident)],
      dat.cca.tmp@active.ident[grepl("CD4 T cells",dat.cca.tmp@active.ident)])
table(idx_reclassify_to_cd8[grepl("CD4 T cells",dat.cca.tmp@active.ident)],
      dat.cca.tmp@meta.data$project.name[grepl("CD4 T cells",dat.cca.tmp@active.ident)])

cells_reclassify <- colnames(dat.cca.tmp@assays$RNA@counts[,idx_reclassify_to_cd8])
dat.cca.tmp <- RenameIdentsCells(dat.cca.tmp, cells_reclassify, "CD8 T cells (unknown)")

idx_reclassify_to_cd4 <- (dat.cca.tmp@assays$RNA@counts["CD4",] > dat.cca.tmp@assays$RNA@counts["CD8A",] & 
                            dat.cca.tmp@assays$RNA@counts["CD4",] > dat.cca.tmp@assays$RNA@counts["CD8B",]) & 
  grepl("CD8 T cells",dat.cca.tmp@active.ident)
table(idx_reclassify_to_cd4[grepl("CD8 T cells",dat.cca.tmp@active.ident)],
      dat.cca.tmp@active.ident[grepl("CD8 T cells",dat.cca.tmp@active.ident)])
table(idx_reclassify_to_cd4[grepl("CD8 T cells",dat.cca.tmp@active.ident)],
      dat.cca.tmp@meta.data$project.name[grepl("CD8 T cells",dat.cca.tmp@active.ident)])

cells_reclassify <- colnames(dat.cca.tmp@assays$RNA@counts[,idx_reclassify_to_cd4])
dat.cca.tmp <- RenameIdentsCells(dat.cca.tmp, cells_reclassify, "CD4 T cells (unknown)")

DimPlot(dat.cca.tmp, label = T, reduction = "tsne",
            cells = WhichCells(dat.cca.tmp,idents = "CD4 T cells (unknown)"))

DimPlot(dat.cca.tmp, label = T, reduction = "tsne",
        cells = WhichCells(dat.cca.tmp,idents = "CD8 T cells (unknown)"))

DimPlot(dat.cca.tmp, label = T, reduction = "tsne",
        cells = WhichCells(dat.cca.tmp,idents = c("CD4 T cells (unknown)","CD8 T cells (unknown)")))

DefaultAssay(dat.cca.tmp) <- "SCT"
dat.cca.tmp <- CellCycleScoring(dat.cca.tmp, s.features = cc.genes$s.genes, 
                                g2m.features = cc.genes$g2m.genes, set.ident = TRUE)

DefaultAssay(dat.cca.tmp) <- "integrated"
dat.cca.tmp.cc <- ScaleData(dat.cca.tmp, vars.to.regress = c("S.Score","G2M.Score"))


#FeaturePlot(dat.cca.tmp,reduction = "tsne",features = c("S.Score","G2M.Score"))

#dat.cca.tmp.cc <- RunPCA(dat.cca.tmp, features = c(cc.genes$s.genes, cc.genes$g2m.genes))

dat.cca.tmp.cc <- RunTSNE(dat.cca.tmp.cc, dims = 1:50)
DimPlot(dat.cca.tmp.cc)
DimPlot(dat.cca.tmp.cc,group.by = "active.ident.updated",label = T,reduction = "tsne")
FeaturePlot(dat.cca.tmp.cc,label = T,features = c("CD4","CD8A"),reduction = "tsne")
DimPlot(dat.cca.tmp.cc,group.by = "active.ident.updated",label = T,reduction = "tsne", 
        cells = rownames(dat.cca.tmp.cc@meta.data)[dat.cca.tmp.cc@meta.data$active.ident.updated %in% c("CD8 T cells (unknown)","CD4 T cells (unknown)")])
DimPlot(dat.cca.tmp.cc,group.by = "active.ident.updated",label = T,reduction = "umap", 
        cells = rownames(dat.cca.tmp.cc@meta.data)[dat.cca.tmp.cc@meta.data$active.ident.updated %in% c("CD8 T cells (unknown)","CD4 T cells (unknown)")])

DimPlot(dat.cca.tmp.cc,group.by = "active.ident.updated",label = T,reduction = "umap", 
        cells.highlight = rownames(dat.cca.tmp.cc@meta.data)[dat.cca.tmp.cc@meta.data$active.ident.updated %in% c("CD8 T cells (unknown)","CD4 T cells (unknown)")])
Idents(dat.cca.tmp.cc) <- dat.cca.tmp.cc@meta.data$active.ident.updated
DimPlot(dat.cca.tmp.cc,group.by = "active.ident.updated",label = T,reduction = "umap", 
        cells.highlight = list(
          CD8=rownames(dat.cca.tmp.cc@meta.data)[dat.cca.tmp.cc@meta.data$active.ident.updated %in% c("CD8 T cells (unknown)")],
          CD4=rownames(dat.cca.tmp.cc@meta.data)[dat.cca.tmp.cc@meta.data$active.ident.updated %in% c("CD4 T cells (unknown)")]),
        cols.highlight = c("darkblue", "darkred")
        )

DimPlot(dat.cca.tmp.cc,group.by = "active.ident.updated",label = T,reduction = "umap", 
        cells.highlight = list(
          CD8=rownames(dat.cca.tmp.cc@meta.data)[grepl("CD8 T cells",dat.cca.tmp.cc@meta.data$active.ident.updated)],
          CD4=rownames(dat.cca.tmp.cc@meta.data)[grepl("CD4 T cells",dat.cca.tmp.cc@meta.data$active.ident.updated)]),
        cols.highlight = c("darkblue", "darkred")
)

DotPlot(subset(dat.cca.tmp.cc,idents=c("CD8 T cells (unknown)","CD4 T cells (unknown)")),
        features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
                     "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
                     "TSPAN13","IL3RA","CLEC4C"),
        cols = c("blue", "red"), dot.scale = 8, assay = "RNA") +
...
dat.cca.tmp.raw <- GetAssayData(dat.cca.tmp, slot = "counts")
dat.cca.tmp.raw <- CreateSeuratObject(dat.cca.tmp.raw, 
                                      meta.data = dat.cca.tmp@meta.data)
dat.cca.tmp.raw <- SplitObject(dat.cca.tmp.raw, split.by = "project.name")

dat.cca.tmp.sct.split <- dat.cca.tmp.raw
rm(dat.cca.tmp.raw)
gc()
...
dat.cca.tmp.sct.split <- FindNeighbors(
  ...
  verbose = T)
dat.cca.tmp.sct.split <- FindClusters(
  dat.cca.tmp.sct.split, resolution = 0.8, verbose = T)

dat.cca.tmp.sct <- dat.cca.tmp.sct.split

#saveRDS(dat.cca.tmp.sct, file = paste0(outdir,"dat.cca.tmp.sct.rda"))

dat.cca.tmp.sct <- readRDS(paste0(outdir,"dat.cca.tmp.sct.rda"))

DimPlot(dat.cca.tmp.sct,reduction = "tsne",group.by = "active.ident.updated",
        label = T, 
        cells.highlight = list(
          CD8=rownames(dat.cca.tmp.cc@meta.data)[grepl("CD8 T cells (unknown)",dat.cca.tmp.cc@meta.data$active.ident.updated,fixed=T)],
          CD4=rownames(dat.cca.tmp.cc@meta.data)[grepl("CD4 T cells (unknown)",dat.cca.tmp.cc@meta.data$active.ident.updated,fixed=T)]),
        cols.highlight = c("darkblue", "darkred")
)

Idents(dat.cca.tmp.sct) <- dat.cca.tmp.sct@meta.data$active.ident.updated

idx_reclassify_to_cd8 <- (dat.cca.tmp.sct@assays$RNA@counts["CD8A",] > dat.cca.tmp.sct@assays$RNA@counts["CD4",] | 
                            dat.cca.tmp.sct@assays$RNA@counts["CD8B",] > dat.cca.tmp.sct@assays$RNA@counts["CD4",]) & grepl("CD4 T cells",dat.cca.tmp.sct@active.ident)

cells_reclassify <- colnames(dat.cca.tmp.sct@assays$RNA@counts[,idx_reclassify_to_cd8])
dat.cca.tmp.sct <- RenameIdentsCells(dat.cca.tmp.sct, cells_reclassify, "CD8 T cells (unknown)")

idx_reclassify_to_cd4 <- (dat.cca.tmp.sct@assays$RNA@counts["CD4",] > dat.cca.tmp.sct@assays$RNA@counts["CD8A",] & 
                            dat.cca.tmp.sct@assays$RNA@counts["CD4",] > dat.cca.tmp.sct@assays$RNA@counts["CD8B",]) & 
  grepl("CD8 T cells",dat.cca.tmp.sct@active.ident)

cells_reclassify <- colnames(dat.cca.tmp.sct@assays$RNA@counts[,idx_reclassify_to_cd4])
dat.cca.tmp.sct <- RenameIdentsCells(dat.cca.tmp.sct, cells_reclassify, "CD4 T cells (unknown)")

idx_cd4_and_cd8 <- (dat.cca.tmp.sct@assays$RNA@counts["CD4",] > 0 & (dat.cca.tmp.sct@assays$RNA@counts["CD8A",] > 0 | 
    dat.cca.tmp.sct@assays$RNA@counts["CD8B",] > 0) & grepl("T cells",dat.cca.tmp.sct@active.ident))
table(idx_cd4_and_cd8)

cells_reclassify <- colnames(dat.cca.tmp.sct@assays$RNA@counts[,idx_cd4_and_cd8])
dat.cca.tmp.sct <- RenameIdentsCells(dat.cca.tmp.sct, cells_reclassify, "CD4+ CD8+ T cells (unknown)")

DimPlot(dat.cca.tmp.sct,reduction = "tsne",group.by = "active.ident.updated",
        label = T)

dat.cca.tmp.sct@meta.data$active.ident.updated.simplified <- dat.cca.tmp.sct@meta.data$active.ident.updated

idx <- grepl("CD8 T cells",dat.cca.tmp.sct@meta.data$active.ident.updated) & 
  ! grepl("CD8 T cells (unknown)",dat.cca.tmp.sct@meta.data$active.ident.updated,fixed = T) & 
  ! grepl("CD4+ CD8+ T cells (unknown)",dat.cca.tmp.sct@meta.data$active.ident.updated,fixed = T)
dat.cca.tmp.sct@meta.data$active.ident.updated.simplified[idx] <- "CD8 T cells (legit)"

idx <- grepl("CD4 T cells",dat.cca.tmp.sct@meta.data$active.ident.updated) & 
  ! grepl("CD4 T cells (unknown)",dat.cca.tmp.sct@meta.data$active.ident.updated,fixed = T) & 
  ! grepl("CD4+ CD8+ T cells (unknown)",dat.cca.tmp.sct@meta.data$active.ident.updated,fixed = T)
dat.cca.tmp.sct@meta.data$active.ident.updated.simplified[idx] <- "CD4 T cells (legit)"

Idents(dat.cca.tmp.sct) <- dat.cca.tmp.sct@meta.data$active.ident.updated.simplified

DimPlot(dat.cca.tmp.sct,reduction = "tsne",group.by = "active.ident.updated.simplified",
        label = T)

DimPlot(dat.cca.tmp.sct,reduction = "tsne",group.by = "active.ident.updated.simplified",
        label = T,
        cells.highlight = list(
          CD8_unknown=rownames(dat.cca.tmp.sct@meta.data)[
            grepl("CD8 T cells (unknown)",dat.cca.tmp.sct@meta.data$active.ident.updated.simplified,fixed=T)],
          CD4_unknown=rownames(dat.cca.tmp.sct@meta.data)[
            grepl("CD4 T cells (unknown)",dat.cca.tmp.sct@meta.data$active.ident.updated.simplified,fixed=T)],
          CD8_legit=rownames(dat.cca.tmp.sct@meta.data)[
            grepl("CD8 T cells (legit)",dat.cca.tmp.sct@meta.data$active.ident.updated.simplified,fixed=T)],
          CD4_legit=rownames(dat.cca.tmp.sct@meta.data)[
            grepl("CD4 T cells (legit)",dat.cca.tmp.sct@meta.data$active.ident.updated.simplified,fixed=T)]),
        cols.highlight = c("darkblue", "darkred","green","yellow"),pt.size = 0.01,shuffle = T
)

DimPlot(dat.cca,reduction = "tsne",group.by = "active.ident.updated",
        label = T,
        cells.highlight = list(
          CD8_unknown=rownames(dat.cca.tmp.sct@meta.data)[
            grepl("CD8 T cells (unknown)",dat.cca.tmp.sct@meta.data$active.ident.updated.simplified,fixed=T)],
...
DimPlot(dat.cca,reduction = "tsne",group.by = "active.ident.updated",
        label = T,
        cells.highlight = list(
          CD8=union(rownames(dat.cca.tmp.sct@meta.data)[
            grepl("CD8 T cells (unknown)",dat.cca.tmp.sct@meta.data$active.ident.updated.simplified,fixed=T)],
            rownames(dat.cca.tmp.sct@meta.data)[
              grepl("CD8 T cells (legit)",dat.cca.tmp.sct@meta.data$active.ident.updated.simplified,fixed=T)]),
          CD4=union(rownames(dat.cca.tmp.sct@meta.data)[
            grepl("CD4 T cells (unknown)",dat.cca.tmp.sct@meta.data$active.ident.updated.simplified,fixed=T)],
            rownames(dat.cca.tmp.sct@meta.data)[
              grepl("CD4 T cells (legit)",dat.cca.tmp.sct@meta.data$active.ident.updated.simplified,fixed=T)])),
        cols.highlight = c("darkblue", "darkred"),pt.size = 0.01,shuffle = T
)

DimPlot(dat.cca,reduction = "tsne",group.by = "project.name",
        label = T)

DimPlot(dat.cca.tmp.sct,reduction = "tsne",group.by = "project.name",
        label = T)


dat.cca.tmp.tils <- subset(dat.cca.tmp.sct, project.name == "G18-023")
meta <- dat.cca.tmp.tils@meta.data
dat.cca.tmp.tils <- GetAssayData(dat.cca.tmp.tils, slot = "counts", assay = "RNA")
dat.cca.tmp.tils <- CreateSeuratObject(dat.cca.tmp.tils, 
                                            meta.data = meta)
dat.cca.tmp.tils <- CellCycleScoring(dat.cca.tmp.tils, s.features = cc.genes$s.genes, 
                                g2m.features = cc.genes$g2m.genes, set.ident = TRUE)
dat.cca.tmp.tils <- SCTransform(dat.cca.tmp.tils, vst.flavor = "v2", verbose = T,
                                vars.to.regress = c("S.Score", "G2M.Score","Sex","Sample.ID"))
dat.cca.tmp.tils <- FindVariableFeatures(dat.cca.tmp.tils)
dat.cca.tmp.tils <- RunPCA(dat.cca.tmp.tils, npcs=50, verbose=T)
dat.cca.tmp.tils <- RunTSNE(dat.cca.tmp.tils, dims = 1:50, verbose = T)
dat.cca.tmp.tils <- FindNeighbors(dat.cca.tmp.tils, reduction = "pca", 
                                  dims = 1:50, n.trees = 100, verbose = T)
dat.cca.tmp.tils <- FindClusters(dat.cca.tmp.tils, resolution = 0.8, verbose = T)

#Idents(dat.cca.tmp.tils) <- dat.cca.tmp.tils$active.ident.updated.simplified
dat.cca.tmp.tils$active.ident.updated.simplified2 <- dat.cca.tmp.tils$active.ident.updated.simplified
dat.cca.tmp.tils$active.ident.updated.simplified2[grepl("CD8 T cells",dat.cca.tmp.tils$active.ident.updated.simplified2)] <- "CD8 T cells"
dat.cca.tmp.tils$active.ident.updated.simplified2[grepl("CD4 T cells",dat.cca.tmp.tils$active.ident.updated.simplified2)] <- "CD4 T cells"
Idents(dat.cca.tmp.tils) <- dat.cca.tmp.tils$active.ident.updated.simplified2

DimPlot(subset(dat.cca.tmp.tils, ident=c("CD8 T cells",
                                         "CD4 T cells")), 
        reduction = "tsne", group.by = "active.ident.updated.simplified2",
        label = T)

DefaultAssay(dat.cca.tmp.tils) <- "RNA"
FeaturePlot(subset(dat.cca.tmp.tils, ident=c("CD8 T cells",
                                         "CD4 T cells")), 
        reduction = "tsne", features=c("CD4","CD8A"),blend = T,order = T)

DimPlot(subset(dat.cca.tmp.tils, ident=c("CD8 T cells",
                                         "CD4 T cells")), 
        reduction = "tsne", group.by = "Sample.ID",
        label = T)

# dat.cca.tmp.tils <- reintegrate(subset(dat.cca.tmp.sct, project.name == "G18-023"),
#                                 sample.tree=NULL,
#                                 vars.to.regress=c("S.Score", "G2M.Score"),
#                                 split.by = "Sample.ID")

dat.cca.tmp.tils <- reintegrate(subset(dat.cca.tmp.sct, project.name == "G18-023"),
                                 sample.tree=make_sample_tree(c(5,7,4,6,3,2,1,8)),
                                 vars.to.regress=c("S.Score", "G2M.Score"),
                                 split.by = "Sample.ID")

Idents(dat.cca.tmp.tils) <- dat.cca.tmp.tils$active.ident.updated.simplified

x <- subset(dat.cca.tmp.tils, ident=c("CD8 T cells",
                                      "CD4 T cells"))
table(x@assays$RNA@counts["CD4",] > 0 | x@assays$RNA@counts["CD8A",] > 0 | 
        x@assays$RNA@counts["CD8B",] > 0 | x@assays$RNA@counts["CD3D",] > 0)

idx <- (x@assays$RNA@counts["CD4",] == 0 & x@assays$RNA@counts["CD8A",] == 0 & 
        x@assays$RNA@counts["CD8B",] == 0) & (x@assays$RNA@counts["CD3D",] > 0 | x@assays$RNA@counts["CD3G",] > 0)
table(idx)

cells <- rownames(x@meta.data)[idx]

pheatmap(x@assays$RNA@counts[c("CD3D","CD3G","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
                               "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1","CD14",
                               "TSPAN13","IL3RA","CLEC4C"),idx],
         show_colnames = F,treeheight_row = 0,treeheight_col = 0)

pheatmap(cr,show_rownames = F,show_colnames = F,annotation_col = annot, 
         treeheight_row = 0, treeheight_col = 0)


dat.cca.tmp.tils$active.ident.updated.simplified2[rownames(dat.cca.tmp.tils@meta.data) %in% cells] <- "CD3+ CD4- CD8- T cells"
Idents(dat.cca.tmp.tils) <- dat.cca.tmp.tils$active.ident.updated.simplified2

DotPlot(subset(dat.cca.tmp.tils,idents=c("CD4 T cells","CD8 T cells",
                                         "CD3+ CD4- CD8- T cells")),
        features = c("CD3D","CD8A","CD8B","CD4","NCAM1","CD14","FCGR3A","CD68",
                     "CD19","MS4A1","MITF","PMEL","MLANA","TYR","HBA1",
                     "TSPAN13","IL3RA","CLEC4C"),
...
DimPlot(subset(dat.cca.tmp.tils, ident=c("CD8 T cells",
                                         "CD4 T cells",
                                         "CD3+ CD4- CD8- T cells")), 
        reduction = "tsne", group.by = "active.ident.updated.simplified2",split.by = "Sample.ID",
        label = T)
...
VlnPlot(dat.cca.tmp.tils,
        group.by = "active.ident.updated.simplified2", features = feats, 
        ncol = 2,raster = T,pt.size = 0,assay = "RNA",slot = "counts") +
  NoLegend() + geom_boxplot(outlier.size=0)
...
# Check different integration orders and include Sex
dat.cca.tmp.3121_cc_sex <- reintegrate(dat.cca.tmp,sample.tree=matrix(c(-3, 1, -2, -1),ncol=2),
                                       vars.to.regress=c("S.Score", "G2M.Score","Sex"))
saveRDS(dat.cca.tmp.3121_cc_sex, file = paste0(outdir,"dat.cca.tmp.3121_cc_sex.rda"))

dat.cca.tmp.2131_cc_sex <- reintegrate(dat.cca.tmp,sample.tree=matrix(c(-2, 1, -3, -1),ncol=2),
                                       vars.to.regress=c("S.Score", "G2M.Score","Sex"))
saveRDS(dat.cca.tmp.2131_cc_sex, file = paste0(outdir,"dat.cca.tmp.2131_cc_sex.rda"))

dat.cca.tmp.3112_cc_sex <- reintegrate(dat.cca.tmp,sample.tree=matrix(c(-3, 1, -1, -2),ncol=2),
                                       vars.to.regress=c("S.Score", "G2M.Score","Sex"))
saveRDS(dat.cca.tmp.3112_cc_sex, file = paste0(outdir,"dat.cca.tmp.3112_cc_sex.rda"))

# Trying Harmony on the TIL dataset --------------------------------------------
library(harmony)
features <- list()
for (sname in unique(dat.cca.tmp.sct@meta.data$Sample.ID[
  dat.cca.tmp.sct@meta.data$Sex=="M" & 
  dat.cca.tmp.sct@meta.data$project.name == "G18-023"])){
  
  dat <- subset(dat.cca.tmp.sct, Sample.ID == sname)
  meta <- dat.cca.tmp.sct@meta.data[dat.cca.tmp.sct$Sample.ID == sname,]
  ...
  dat <- FindVariableFeatures(dat, selection.method = "vst", 
                                           nfeatures = 4000)
  features[[sname]] <- VariableFeatures(dat)
}
...
FeaturePlot(dat, features = union(grep("^TRGV",rownames(dat@assays$RNA@data),value = T),
            ...
            min.cutoff = 0,slot = "counts",order = T)
...
pc <- Embeddings(dat, 'pca')
Idents(dat) <- dat@meta.data$active.ident.updated.simplified2

markers_t <- FindMarkers(dat,ident.1 = "CD8 T cells",ident.2 = "CD4 T cells")

dat <- RunPCA(dat, features = rownames(markers_t), 
              npcs = 20, verbose = T,reduction.name="pca")
dat <- RunUMAP(dat, dims = 1:20, reduction = "pca")
dat <- FindNeighbors(dat, dims = 1:20,reduction = "pca")
DimPlot(dat,
        reduction = "umap", group.by = "active.ident.updated.simplified2",
        label = T)

d <- aggregate(t(dat@assays$RNA@data[rownames(markers_t),]),list(dat@meta.data$active.ident.updated.simplified2),mean)
rownames(d) <- d$Group.1
d$Group.1 <- NULL
pheatmap(d)

dat.cca.tmp.tils <- subset(dat.cca.tmp.sct, project.name == "G18-023" & Sex == "M")
meta <- dat.cca.tmp.tils@meta.data
dat.cca.tmp.tils <- GetAssayData(dat.cca.tmp.tils, slot = "counts", assay = "RNA")
dat.cca.tmp.tils <- CreateSeuratObject(counts = dat.cca.tmp.tils, meta.data = meta)
dat.cca.tmp.tils <- NormalizeData(dat.cca.tmp.tils,verbose = T)



dat.cca.tmp.tils <- FindVariableFeatures(dat.cca.tmp.tils, selection.method = "vst", 
                                         nfeatures = 2000)
dat.cca.tmp.tils <- ScaleData(dat.cca.tmp.tils, verbose = T)
dat.cca.tmp.tils <- RunPCA(dat.cca.tmp.tils, pc.genes = dat.cca.tmp.tils@var.genes, 
                           npcs = 30, verbose = T)
dat.cca.tmp.tils <- RunHarmony(dat.cca.tmp.tils, group.by.vars = "Sample.ID", 
                               plot_convergence = TRUE)

dat.cca.tmp.tils <- RunUMAP(dat.cca.tmp.tils, reduction = "harmony", dims = 1:30)
dat.cca.tmp.tils <- RunTSNE(dat.cca.tmp.tils, reduction = "harmony", dims = 1:30, verbose=T)
dat.cca.tmp.tils <- FindNeighbors(dat.cca.tmp.tils, reduction = "harmony", dims = 1:30)
dat.cca.tmp.tils <- FindClusters(dat.cca.tmp.tils, resolution = 0.5)


DimPlot(dat.cca.tmp.tils,
        reduction = "harmony", group.by = "active.ident.updated.simplified2",
        label = T, dims = c(8,9))

DimPlot(dat.cca.tmp.tils,
        reduction = "umap", group.by = "Sample.ID",
        label = T)
...
Idents(dat.cca.tmp.tils) <- dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2
DimPlot(subset(dat.cca.tmp.tils, ident=c("CD8 T cells",
                                 "CD4 T cells")),
        reduction = "umap", group.by = "active.ident.updated.simplified2",
        label = T)
...
har <- Embeddings(dat.cca.tmp.tils, 'harmony')
pc <- Embeddings(dat.cca.tmp.tils, 'pca')

identical(rownames(har),rownames(dat.cca.tmp.tils@meta.data))

dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2[order(har[,1],decreasing = T)]


plot(cumsum(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2[
  order(har[,9],decreasing = T)]=="CD4 T cells"))

plot(cumsum(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2[
  order(har[,9],decreasing = T)]=="NK cells (II)"))

library(fgsea)
pathways <- list()
for (ct in unique(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2)){
  pathways[[ct]] <- rownames(dat.cca.tmp.tils@meta.data)[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 == ct]
}

component_enrichment <- list()
#for (component in 1:ncol(har)){
for (component in 16:ncol(har)){
  print(component)
  ranks <- har[order(har[,component],decreasing = T),component]
  fgseaRes <- fgsea(pathways = pathways, 
                    stats    = ranks,
                    minSize  = 1,
                    maxSize  = Inf, nproc = 15)
  fgseaRes <- fgseaRes[order(fgseaRes$NES,decreasing = T)]
  component_enrichment[[component]] <- fgseaRes[,1:7]
}

library(psych)
color = sample(grDevices::colors()[grep('gr(a|e)y', grDevices::colors(), invert = T)],
               length(unique(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2)))
pdf(paste0(outdir,"harmony_components.pdf"),width = 20,height = 20)
pairs.panels(har[,1:30],
             gap=0,
             bg = color[factor(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2)],
             pch=21,
             smooth = F,density = F,ellipses = F,hist.col = NA,rug = F)
dev.off()

#har.use <- c(2,4,7,8,9,10,11,12,13,14,17,18,19,21,24,26,28)
har.use <- c(4,25)
dat.cca.tmp.tils <- RunTSNE(dat.cca.tmp.tils, reduction = "harmony", dims = har.use)
dat.cca.tmp.tils <- RunUMAP(dat.cca.tmp.tils, reduction = "harmony", dims = har.use)
dat.cca.tmp.tils <- FindNeighbors(dat.cca.tmp.tils, reduction = "harmony", dims = har.use)
DimPlot(dat.cca.tmp.tils,
        reduction = "tsne", group.by = "active.ident.updated.simplified2",
        label = T)
DimPlot(subset(dat.cca.tmp.tils, ident=c("CD8 T cells",
...
tmp <- cbind(cor(har,dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2=="CD8 T cells"),
             cor(har,dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2=="CD4 T cells"))
tmp <- as.data.frame(tmp)
tmp$component <- rownames(tmp)
head(tmp)

ggplot(tmp,aes(x=V1,y=V2,color=component)) + 
  geom_text(aes(label=component)) + xlab("CD8") + ylab("CD4")

png(paste0(outdir,"pc_components.png"),width = 10000,height = 10000,units = "px")
pairs.panels(pc[,1:30],
             gap=0,
             bg = color[factor(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2)],
             pch=21,
             smooth = F, density = F, ellipses = F, hist.col = NA, rug = F)
...
png(paste0(outdir,"pc_components.t.png"),width = 10000,height = 10000,units = "px")
pairs.panels(pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),1:30],
             gap=0,
             bg = color[factor(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells")])],
             pch=21,
             smooth = F, density = F, ellipses = F, hist.col = NA, rug = F)
dev.off()

#pcs.keep <- c(4,9,11,13,14,17,20,22) #13,20
pcs.keep <- c(4,9,11,14,17,20,22) #13,20. 11 seems sex related

png(paste0(outdir,"pc_components.t.sex.png"),width = 10000,height = 10000,units = "px")
pairs.panels(pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),1:30],
             gap=0,
             bg = color[factor(dat.cca.tmp.tils@meta.data$Sex[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells")])],
             pch=21,
             smooth = F, density = F, ellipses = F, hist.col = NA, rug = F)
dev.off()

pcs.sex <- c(11,12,19,25)

png(paste0(outdir,"pc_components.t.sample.png"),width = 10000,height = 10000,units = "px")
pairs.panels(pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),1:30],
             gap=0,
             bg = color[factor(dat.cca.tmp.tils@meta.data$Sample.ID[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells")])],
             pch=21,
             smooth = F, density = F, ellipses = F, hist.col = NA, rug = F)
dev.off()

pcs.sample <- c(4,8,9,10,11,13,14,15,16,18,19,21,25,26,27,30)

#pcs.keep <- setdiff(setdiff(pcs.keep,pcs.sex),pcs.sample)
pcs.keep <- setdiff(pcs.keep,pcs.sex)


pairs.panels(pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),pcs.keep],
             gap=0,
             bg = color[factor(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells")])],
             pch=21,
             smooth = F, density = F, ellipses = F, hist.col = NA, rug = F)

x <- pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),pcs.keep]

library("scatterplot3d")
colors <- color[factor(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells")])]
scatterplot3d(pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),pcs.keep], 
              pch = 16, color=colors,angle = 50)

scatter3d(x = pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),pcs.keep[1]], 
          y = pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),pcs.keep[2]], 
          z = pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),pcs.keep[3]], 
          groups = factor(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells")]),
          surface=FALSE, ellipsoid = TRUE)

rgl::plot3d(x = pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),pcs.keep[1]], 
          y = pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),pcs.keep[2]], 
          z = pc[dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2 %in% c("CD4 T cells","CD8 T cells"),pcs.keep[3]])

dat.cca.tmp.tils <- RunHarmony(dat.cca.tmp.tils, group.by.vars = "Sample.ID", 
                               plot_convergence = TRUE,dims.use = pcs.keep)
                               #epsilon.cluster = -Inf, epsilon.harmony=-Inf)
dat.cca.tmp.tils <- RunUMAP(dat.cca.tmp.tils, reduction = "harmony", dims = 1:30, verbose = T)
dat.cca.tmp.tils <- RunTSNE(dat.cca.tmp.tils, reduction = "harmony", dims = 1:30, verbose = T)
dat.cca.tmp.tils <- FindNeighbors(dat.cca.tmp.tils, reduction = "harmony", dims = 1:30)
dat.cca.tmp.tils <- FindClusters(dat.cca.tmp.tils, resolution = 0.5)
...
# CD4 dominated: S1, S3
# CD8 dominated: S7, S8, S9

#x <- aggregate(pc,list(dat.cca.tmp.tils@meta.data$active.ident.updated.simplified2),mean)
x <- aggregate(pc,list(dat@meta.data$active.ident.updated.simplified2),mean)
rownames(x) <- x$Group.1
x$Group.1 <- NULL
x$group <- rownames(x)
x <- reshape2::melt(x)
x <- x[x$group %in% c("CD4 T cells","CD8 T cells"),]

pc.x <- as.data.frame(pc)
pc.x$group <- dat@meta.data$active.ident.updated.simplified2
ggplot(pc.x,aes(x=PC_16,y=PC_2,fill=group)) + geom_point(aes(color=group),size=0.1)

ggplot(x,aes(x=variable,y=value,fill=group)) + geom_bar(stat="identity",position="dodge")

pcs.keep <- c(1,2,3,4,7,8,9,10,11,12,14,15,16)

x <- aggregate(pc,list(dat.cca.tmp.tils@meta.data$Sample.ID),mean)
rownames(x) <- x$Group.1
x$Group.1 <- NULL
...
x <- aggregate(pc,list(dat.cca.tmp.tils@meta.data$Sex),mean)
rownames(x) <- x$Group.1
x$Group.1 <- NULL
#pheatmap(x,border_color = NA)
x$group <- rownames(x)

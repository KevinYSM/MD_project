...
vdjdb.simplified <- vdjdb[,c("Gene","CDR3","V","J","Epitope.gene","Species","Epitope.species","complex.id")]
...
vdjdb.simplified <- vdjdb.simplified[,c("CDR3.alpha","CDR3.beta","TRAV","TRAJ","TRBV","TRBJ","TRBD",
                    "Epitope.gene","Epitope.species","Species","complex.id")]
...
for (sname in names(db.matches)){
  ...
  db.matches[[sname]]$project.name[db.matches[[sname]]$project.name=="G18-023"] <- "TIL"
  db.matches[[sname]]$project.name[db.matches[[sname]]$project.name=="GWA-JN-388"] <- "Biopsy"
}
...
m <- m[rn,]
pdf(file="~/proj/um_ss/Manuscript/tcr_database_matches.pdf",width = 9.5,height = 10)
...
library(WriteXLS)
WriteXLS(db_matches.vasu,ExcelFileName = "~/proj/um_ss/Investigations/db_matches.vasu.xlsx",

...
lig.tumor.cellbender <- rliger::createLiger(fnames)
...
idx_keep <- (lig.tumor@cell.data[["nGene"]] > 500 | lig.tumor@cell.data[["gene_val"]] > 0) & 
  lig.tumor@cell.data[["nUMI"]] > 500 & 
...
lig.tumor@cell.data$cells_filtered <- ! rownames(lig.tumor@cell.data) %in% cells_keep
table(lig.tumor@cell.data$cells_filtered)["TRUE"]/nrow(lig.tumor@cell.data)
table(lig.tumor@cell.data$cells_filtered,
      lig.tumor@cell.data$dataset)

lig.tumor@cell.data$cells_qc_filtered <- ! rownames(lig.tumor@cell.data) %in% rownames(lig.tumor@cell.data[idx_keep,])

rliger::plotFeature(object = lig.tumor, feature = "cells_filtered", by.dataset = F, discrete = T)
rliger::plotFeature(object = lig.tumor, feature = "cells_qc_filtered", by.dataset = F, discrete = T)

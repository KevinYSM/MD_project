...
dat.cca <- AddMetaData(dat.cca, metadata = dat.cca.vasu_phenotype, 
                       col.name = "vasu_phenotype")
...
tils.unintegrated <- lapply(tils.unintegrated, function(x){
  x <- AddMetaData(x, metadata = dat.cca.vasu_phenotype, 
                   col.name = "vasu_phenotype")  
})
...
p1 <- DimPlot(tils.unintegrated[["UM1"]],
              ...
              cols = c("gray", "gray", "blue","red"), order = T) + 
...
pdf(file = "~/proj/um_ss/Manuscript/tils_cell_type.pdf", width = 50, height = 7)
...
WriteXLS(res,
         ExcelFileName = "~/proj/um_ss/Manuscript/proportions_til_vs_vasu_matches_per_cluster.xlsx",
...
ggsave(filename = paste0(
  "~/proj/um_ss/Manuscript/proportions_til_vs_vasu_matches_per_cluster.pdf"),
       width = 20, height = 7)
...
get_cells_matching_mart1_vasu_and_db <- function(dat.tumor_til.vdj, db_matches.vasu, sname_1){
  ...
  um_id <- unique(dat.cca@meta.data$UM.ID[
    dat.cca@meta.data$Sample.ID == sname_1])
  ...
  if (length(matches_cdr3) > 0){
    ...
    matches_cdr3.small <- unique(matches_cdr3[,
      c("tenX.cdr3", "db.Antigen", "db.Antigen_species_or_pathology", "cell")])
...
names(cells_matching_mart1_vasu_and_db) <- gsub(
  "SampleID_[0-9]_11june18_", "", names(cells_matching_mart1_vasu_and_db))
...
for (sname in names(tils.unintegrated)){
  ...
  tmp <- factor(tmp, levels = c("None", "Unidentified", "CD3+41bb+", "MART1", 
                                "MART1_db"))
...
dat.cca.til_tumor <- subset(dat.cca, subset = project.name %in% 
                              c("G18-023", "GWA-JN-388"))
...
rm(dat.cca.til_tumor)
gc()

tcr_til_tumors <- get_tcr_matrix(dat.cca.til_tumor.with_cdr3)

tcr <- tcr_til_tumors[rownames(tcr_til_tumors) %in% 
...
ranks <- lapply(split(vdj.takara,paste0(vdj.takara$UM.ID,":",vdj.takara$chains)),

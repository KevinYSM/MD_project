...
reorder_sankey_factors <- function(df.sankey){
  subject <- 
...
df.sankey.biopsy_til.subcutaneous <- format_for_ggsankey(tcrs.split_ab.merged = df.subcutaneous,
                                                  cols_include = c("cell","subject","condition", 
                                                                   "antigen", "organism", "human_antigen"),
                                                  cols_plot = c("subject","condition",
                                                                "organism","human_antigen"))

df.sankey.biopsy_til.liver <- format_for_ggsankey(tcrs.split_ab.merged = df.liver,
                                            cols_include = c("cell","subject","condition", 
                                                             "antigen", "organism", "human_antigen"),
                                            cols_plot = c("subject","condition",
                                                          "organism","human_antigen"))

df.sankey.biopsy_til.subcutaneous <- reorder_sankey_factors(df.sankey.biopsy_til.subcutaneous)
df.sankey.biopsy_til.liver <- reorder_sankey_factors(df.sankey.biopsy_til.liver)

plot_sankey(df.sankey.biopsy_til.subcutaneous)
plot_sankey(df.sankey.biopsy_til.liver)

...
markers.cd8_t <- as.data.frame(read_excel(paste0(outdir, "markers.cd8_t.xlsx")), 
                               stringsAsFactors = F)
...
x <- seu.filt.cd8_t
Idents(x) <- x@meta.data$Mouse_biopsy_site_and_TIL_selection
markers.find_all <- FindAllMarkers(x)

WriteXLS(markers.find_all, ExcelFileName = paste0(outdir, "markers.find_all.cd8_t.xlsx"),
         BoldHeaderRow = T, AutoFilter = T, AdjWidth = T)

markers.find_all.sig <- markers.find_all[
  which(markers.find_all$p_val_adj < 0.05),]

res <- list()
n <- 25
for (cluster in unique(markers.find_all.sig$cluster)){
  tmp <- markers.find_all.sig[which(markers.find_all.sig$cluster==cluster),]
  tmp <- tmp[order(tmp$avg_log2FC,decreasing = T),]
  res[[cluster]] <- head(tmp$gene,n=n)
}

pdf(file= paste0(outdir,"dotplot_markers_Mouse_biopsy_site_and_TIL_selection.pdf"), 
    width = 15, height = 3.5)
DotPlot(x, features = unique(unlist(res)), cluster.idents = T, assay = "RNA", 
        scale = T) + vertical_xlabels
dev.off()

# Differential expression analysis (pseudobulk) (simplified) -------------------
library(tidyverse)
library(magrittr)
library(Matrix)
library(purrr)
...
get_pseudobulk <- function(seu.filt.cd8_t, group_labels, group_var, sample_id_var = "Sample.ID"){
  cells <- rownames(seu.filt.cd8_t@meta.data)[
    seu.filt.cd8_t@meta.data[[group_var]] %in% group_labels]
  
  seu.filt.cd8_t@meta.data$is_set <- rownames(seu.filt.cd8_t@meta.data) %in% cells
  seu.filt.cd8_t.set <- subset(seu.filt.cd8_t, is_set==T)
  
  counts <- seu.filt.cd8_t.set@assays$RNA@counts
  metadata <- seu.filt.cd8_t.set@meta.data
  
  sce <- SingleCellExperiment(assays = list(counts = counts), 
                              colData = metadata)
  
  groups <- colData(sce)[, sample_id_var]
  pb <- aggregate.Matrix(t(counts(sce)), groupings = groups, fun = "sum") 
  rownames(pb) #table(seu.filt.cd8_t@meta.data$Mouse_biopsy_site_and_TIL_selection,seu.filt.cd8_t@meta.data$Sample.ID)
  
  cluster_counts <- t(pb)
  cluster_metadata <- unique(data.frame(Sample.ID = metadata[[sample_id_var]], 
                                 group_id = metadata[[group_var]]))
  rownames(cluster_metadata) <- cluster_metadata$Sample.ID
  cluster_metadata$Sample.ID <- NULL
...
#group_labels <- c("Liver_MART1","Liver_Bulk")

run_diff_expr <- function(seu.filt.cd8_t, group_labels, group_var, 
                          sample_id_var = "Sample.ID", outdir){
  pseudobulk <- get_pseudobulk(seu.filt.cd8_t, 
                               group_labels = group_labels, 
                               group_var = group_var, 
                               sample_id_var = sample_id_var)
  
  dds <- DESeqDataSetFromMatrix(pseudobulk$cluster_counts, 
                                colData = pseudobulk$cluster_metadata, 
                                design = ~ group_id)
  
  dds <- DESeq(dds, test = "LRT", reduced = ~ 1)
  resultsNames(dds)
  
  contrast <- c("group_id", group_labels)
  res <- results(dds, 
                 contrast = contrast,
                 alpha = 0.05)
  ...
  g1 <- DESeq2::plotPCA(rld, intgroup = "group_id")
  
  if (nrow(res.sig)>0){
    # Plot expression heatmap
    normalized_counts <- as.data.frame(counts(dds, normalized = TRUE))
    
    sig_norm <- normalized_counts[rownames(res.sig),]
    
    # Heatmap
    
    g2 <- pheatmap(sig_norm, border_color = NA, scale = "row", 
             annotation = pseudobulk$cluster_metadata)
  } else {
    g2 <- NULL
  }
  
  WriteXLS(res, ExcelFileName = paste0(outdir, paste0(contrast,collapse = "_"), ".xlsx"), 
           AutoFilter = T, AdjWidth = T, BoldHeaderRow = T)
...
pairs <- unique(seu.filt.cd8_t@meta.data$Mouse_biopsy_site_and_TIL_selection)
pairs <- combn(pairs,2)

de <- list()
for (i in 1:ncol(pairs)){
  group_labels <- pairs[,i]
  de[[paste0(group_labels,collapse = ":")]] <- run_diff_expr(
    seu.filt.cd8_t, 
    group_labels = group_labels, 
    group_var = "Mouse_biopsy_site_and_TIL_selection", 
    sample_id_var = "Sample.ID", 
    outdir = outdir)
}
...
# pseudobulk <- get_pseudobulk(seu.filt.cd8_t, 
#                              group_labels = unique(seu.filt.cd8_t@meta.data$Mouse_biopsy_site_and_TIL_selection), 
#                              group_var = "Mouse_biopsy_site_and_TIL_selection", 
#                              sample_id_var = "Sample.ID")
# 
# pseudobulk$cluster_metadata$tissue <- str_split_fixed(pseudobulk$cluster_metadata$group_id,"_",2)[,1]
# pseudobulk$cluster_metadata$selection <- str_split_fixed(pseudobulk$cluster_metadata$group_id,"_",2)[,2]
# 
# dds <- DESeqDataSetFromMatrix(pseudobulk$cluster_counts, 
#                               colData = pseudobulk$cluster_metadata, 
#                               design = ~ tissue + selection)
# 
# dds <- DESeq(dds, test = "LRT", reduced = ~ selection)
# resultsNames(dds)
# 
# #contrast <- c("selection", "MART1", "Bulk")
# contrast <- c("tissue", "Liver", "Spleen")
# 
# res <- results(dds, 
#                contrast = contrast,
#                alpha = 0.05)
...
# Differential expression analysis (old) ---------------------------------------

...
vertical_xlabels <- theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
lig.t.plots <- plotByDatasetAndCluster(
  lig.t, axis.labels = c('UMAP 1', 'UMAP 2'), 
  return.plots = T, clusters = vasu_phenotype)
lig.t.plots[[2]]
...
#  "Exhausted / dysfunctional, proliferative",



DotPlot(s.t,features = 
          marker_features,
        cluster.idents = F,assay = "RNA") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

vasu_phenotype <- readRDS("~/proj/um_ss/Investigations/seurat/results/liger_all/vasu_phenotype.rda")
vasu_phenotype

vasu_phenotype.t <- vasu_phenotype[intersect(names(vasu_phenotype),str_split_fixed(rownames(s.t@meta.data),"GEX_",2)[,2])]
stopifnot(identical(names(vasu_phenotype.t),str_split_fixed(rownames(s.t@meta.data),"GEX_",2)[,2]))
...
table(s.t@meta.data$vasu_phenotype)
s.t@meta.data$vasu_phenotype <- factor(s.t@meta.data$vasu_phenotype,
                                       levels=c("None","Unidentified","CD3+41bb+","MART1"))

DimPlot(subset(s.t,vasu_phenotype %in% c("CD3+41bb+","MART1")),
        label = T,group.by = "vasu_phenotype",
        order = F, cols = c("red", "blue", "gray","gray"))
...
DimPlot(subset(s.t,orig.ident %in% c("A2-GEX","C7-GEX","D4-GEX")),
        label = T, group.by = "vasu_phenotype",
        order = T, cols = c("gray", "gray", "blue","red"), split.by = "orig.ident")
dev.off()
...
DimPlot(s.t,
        label = T, group.by = "vasu_phenotype",
        order = T, cols = c("gray", "green", "blue","red"),split.by = "orig.ident")
...
DimPlot(subset(dat.tumor,UM.ID %in% c("UM1","UM2","UM3","UM9","UM19","UM24")), 
        split.by = "UM.ID", group.by = "vasu_phenotype",
        cols = c("gray", "green", "blue","red"),order = T) + 
DimPlot(subset(dat.til,UM.ID %in% c("UM1","UM2","UM3","UM9","UM19","UM24")), 
        split.by = "UM.ID", group.by = "vasu_phenotype",
        cols = c("gray", "green", "blue","red"),order = T)
...
DimPlot(tils.unintegrated[[1]])
tils.unintegrated[[1]]@active.ident
...
"SampleID_2_11june18_2_AAGTCTGGTCCAGTGC_1"
...
get_tcr_matrix <- function(s.t){
  ...
  for (cell in cells){
    tcr[cell,colnames(tcr) %in% cdr3[[cell]]] <- T
    tcr[cell,colnames(tcr) %in% v_gene[[cell]]] <- T
    tcr[cell,colnames(tcr) %in% j_gene[[cell]]] <- T
    tcr[cell,colnames(tcr) %in% d_gene[[cell]]] <- T
    tcr[cell,colnames(tcr) %in% c_gene[[cell]]] <- T
  }
  tcr[is.na(tcr)] <- F
...
#intersect(rownames(dat.cca@meta.data)[dat.cca@meta.data$project.name == "G18-023"],
#          rownames(dat.cca@meta.data)[dat.cca@meta.data$project.name == "GWA-JN-388"])
...
# Show sample contribution to each cluster -------------------------------------

make_pies <- function(tab){
  ...
  tab$UM.ID <- rownames(tab)
  tab$UM.ID <- factor(tab$UM.ID,levels=paste0("UM",sort(as.numeric(gsub("UM","",unique(tab$UM.ID))))))
  ...
  print(ggplot(tab.melt, aes(x = 1, y = value, fill = UM.ID)) +
...
tab <- table(s.t@meta.data$UM.ID,s.t@active.ident)
make_pies(tab)
ggsave(filename = "~/proj/um_ss/Manuscript/cluster_sample_contribution.t.pie.pdf",width = 15,height = 3)


tab <- table(s@meta.data$UM.ID,s@active.ident)
make_pies(tab)
ggsave(filename = "~/proj/um_ss/Manuscript/cluster_sample_contribution.all.pie.pdf",width = 15,height = 3)

tab <- table(s@meta.data$UM.ID,s@active.ident)
tab <- make_pies(tab)
tab$UM.ID <- NULL
pheatmap(cor(tab,method = "spearman"),border_color = NA)
...
pdf(file = "~/proj/um_ss/Manuscript/cluster_proportion_correlation.all.pdf",width = 5.5,height = 5)
cluster_proportion_correlation.all <- plot_cluster_proportion_correlation(tab,correction = "BH")
dev.off()
...
pdf(file = "~/proj/um_ss/Manuscript/cluster_proportion_correlation.all_no_melanoma.pdf",width = 5.5,height = 5)
cluster_proportion_correlation.all_no_melanoma <- plot_cluster_proportion_correlation(tab,correction = "BH")
dev.off()

tab <- table(s.t@meta.data$UM.ID,s.t@active.ident)
make_pies2(tab)
ggsave(filename = "~/proj/um_ss/Manuscript/cluster_sample_contribution.t.pie2.pdf",width = 15,height = 3)

tab <- table(s@meta.data$UM.ID,s@active.ident)
make_pies2(tab)
ggsave(filename = "~/proj/um_ss/Manuscript/cluster_sample_contribution.all.pie2.pdf",width = 15,height = 3)
...
matches_pair[["C7-GEX:SampleID_9_11june18"]]

dat.tumor_til.vdj[["C7-GEX"]]$vdj.10x_cdr3[["4_AAACGGGAGACCTTTG_3"]]
dat.tumor_til.vdj[["SampleID_9_11june18"]]$vdj.10x_cdr3[["8_CTTTGCGAGGAATCGC_1"]]
...
head(matches_pair.nr)

#unique_samples <- sort(unique(c(matches_pair.nr$sample_1,matches_pair.nr$sample_2)))
...
# Mathces only on CDR3 and only for subset of Vasu TCRs occuring twice or more ----
library(foreach)
library(doParallel)
cores=8
cl <- makeCluster(cores) #not to overload your computer
...
overlap_tcr_vasu_tra <- matrix(NA,nrow=length(snames),ncol=length(snames),dimnames=list(snames,snames))
for (sname_1 in snames){
  for (sname_2 in snames){
    overlap_tcr_vasu_tra[sname_1,sname_2] <- length(intersect(unique(vdj.takara$cdr3[vdj.takara$UM.ID==sname_1 & vdj.takara$chains=="TRA"]),
                                                          unique(vdj.takara$cdr3[vdj.takara$UM.ID==sname_2 & vdj.takara$chains=="TRA"])))
  }
}

snames <- c("UM1","UM9","UM22","UM46")
overlap_tcr_vasu_trb <- matrix(NA,nrow=length(snames),ncol=length(snames),dimnames=list(snames,snames))
for (sname_1 in snames){
  for (sname_2 in snames){
    overlap_tcr_vasu_trb[sname_1,sname_2] <- length(intersect(unique(vdj.takara$cdr3[vdj.takara$UM.ID==sname_1 & vdj.takara$chains=="TRB"]),
